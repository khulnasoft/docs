---
title: "Run Teleflow in local machine"
description: "Prerequisites and steps to run Teleflow in local machine. Learn how to set up Teleflow on your local environment for testing and development."
---
...
<Note>We recommend having at least 8GB of RAM to run Teleflow on a local machine as Teleflow has multiple services running together with external services like redis, mongodb etc.</Note>
...
<Tabs>
    <Tab title="Teleflow Org">
        ```shell
       git clone https://github.com/khulnasoft/teleflow.git
       ````
    </Tab>
...
```shell
cd teleflow && npm run setup:project
```
...
- **start:node** - Runs the `@teleflow/node` package in watch mode
- **start:notification-center** - Runs and builds the React package for the Teleflow notification center
...
The command `npm run setup:project` creates default environment variables that are required to run Teleflow in a development environment. However, if you want to test certain parts of Teleflow or run it in production mode, you need to change some of them. These are all the available environment variables:
...
<AccordionGroup>
  <Accordion title="API Backend">
    ...
    - `FRONT_BASE_URL`The base url on which your frontend is accessible for the user. (e.g. web.teleflow.khulnasoft.com)
    ...
    - `TELEFLOW_API_KEY`The api key of web.teleflow.khulnasoft.com used to send various emails
  ...
  <Accordion title="Worker">
    ...
    - `MAX_TELEFLOW_INTEGRATION_MAIL_REQUESTS`The number of free emails that can be sent with the Teleflow email provider
    - `TELEFLOW_EMAIL_INTEGRATION_API_KEY`The Teleflow email provider Sentry API key
...
The tests create a new instance of Teleflow and a test db and run the tests against it. The test db is removed after all tests have finished running.
...
As an option in our script runner `Jarvis` we have made available an option to run [this script](https://github.com/khulnasoft/teleflow/blob/2f2abdcaaad8a7735e0a2d488607c3276c8975fd/scripts/dev-environment-setup.sh) that will automatically try to install all the dependencies needed to be able to run Teleflow locally, as the previous step of installing the project dependencies through `pnpm install`. When executing it inside `Jarvis`, you will need to have previously installed by yourself `git` and `node`, as we mentioned earlier on this page.
...
- Will install the required [Node.js](https://nodejs.org/en/) version to be able to [run Teleflow](https://github.com/khulnasoft/teleflow/blob/2f2abdcaaad8a7735e0a2d488607c3276c8975fd/package.json#L180)
- Will install [PNPM](https://pnpm.io/) as a package manager, required dependency for some of the tasks inside Teleflow's scripts
...
- Will install the [AWS CLI](https://aws.amazon.com/cli/) tool (not required to run Teleflow; it is a core maintainer used tool)
- Will create a local development domain `local.teleflow.khulnasoft.com` in your local machine
- Will clone the Teleflow repository in your local machine (skippable step) to a selected folder `$HOME/Dev`
