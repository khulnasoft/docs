---
title: "MacOS/Linux"
description: "Learn how to configure your local machine to run Teleflow"
---
...
<Frame>
    <img src="/images/community/machine-setup/aws-cli.png" alt="AWS CLI" />
</Frame>
...
2. Create local S3 buckets needed for the app:
   ...
   ```shell
   aws --endpoint-url=http://localhost:4566 s3 mb s3://teleflow-local
   ```

   In **test** environment:

   ```shell
   aws --endpoint-url=http://localhost:4566 s3 mb s3://teleflow-test
   ```

3. Update cors:

   ```json
   aws --endpoint-url=http://localhost:4566 s3api put-bucket-cors --bucket teleflow-local --cors-configuration file://cors.json
   ```
...
This will allow you to use [local.teleflow.khulnasoft.com](http://local.teleflow.khulnasoft.com) instead of [localhost](http://localhost) when accessing the service on your machine.
...
And add the [local.teleflow.khulnasoft.com](http://local.teleflow.khulnasoft.com) domain

```shell
127.0.0.1       local.teleflow.khulnasoft.com
```
...
<Frame>
    <img src="/images/community/machine-setup/dock.png" alt="Dock" />
</Frame>
