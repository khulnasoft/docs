...
<br/>
You can find the entire code ( front-end as well as back-end ) of this app [here.](https://github.com/khulnasoft/teleflow-headless-demo-app)

## What is the headless notification center
The headless version of Teleflow's notification library package provides users with a lightweight solution for integrating notification functionality into their web applications. 
<br/>
...
1. A Teleflow account. [Sign up for free](http://web.teleflow.khulnasoft.com/?utm_campaign=docs-gs-guides-headless) if you don’t have one yet.
2. A working dev environment in the framework of your choice (or vanilla JS). Here, we'll use a React environment

## Workflow set up in Teleflow
Once, you have signed up for Teleflow and have a dev environment, follow the steps below:
...
1. `applicationIdentifier`: You can find this from the `settings` menu in the [Teleflow web dashboard.](https://web.teleflow.khulnasoft.com/settings?utm_campaign=docs-guides-headless)
...
1. Install the Teleflow package. Open your terminal and run the following command:
```bash
npm install @teleflow/node
```
Once installed, you can import Teleflow into your app and initialize it using your Teleflow account credentials. This step establishes a connection between your app and the Teleflow notification service.
```bash
import { Teleflow } from '@teleflow/node';
const teleflow = new Teleflow('<TELEFLOW_API_KEY>');
```
<Note>
Replace the `<TELEFLOW_API_KEY>` value with the authentic key from the API Key section of your [Teleflow Dashboard.](http://web.teleflow.khulnasoft.com/?utm_campaign=docs-guides-headless)
</Note>
<Note>
🔑 Please do not hardcode your credentials in a file in production. Use environment variables instead.
</Note>

Using the above info, we'll write a function that will help us interact with Teleflow's notification system:
```javascript
import { Teleflow } from '@teleflow/node';

export const notification = async (description) => {
    const teleflow = new Teleflow("<TELEFLOW_API_KEY>");
    await teleflow.subscribers.identify(process.env.SUSBSCRIBER_ID, {
        firstName: 'newSubForHeadless',
    });

// code snippet copied when creating the workflow
    await teleflow.trigger('headless-demo', {
        to: {
...
```javascript
import { notification } from "../teleflow/teleflow.js";
...
<br/>
You can refer to the entire context [here](https://github.com/khulnasoft/teleflow-headless-demo-app/blob/main/frontend/src/context/NotificationContext.js) for a better understanding.
...
Finally, we've used some css to style the app to our liking. You can see that [here.](https://github.com/khulnasoft/teleflow-headless-demo-app/tree/main/frontend/src/styles).
## Conclusion
...
Again, don't forget that you can view the entire code [here](https://github.com/khulnasoft/teleflow-headless-demo-app/tree/main) and if you've any questions, either related to this headless app or something else, feel free to ping use [here.](https://discord.gg/teleflow)
