---
title: 'How to use Teleflow to send notifications to a channel in a Discord server'
description: 'Learn to send Discord notifications swiftly with Teleflow'
---

# Introduction

In this guide, you'll learn how to use Teleflow to send notifications to any channel in a Discord server. But before coding anything up, we just need to go over a couple of setup steps.
The corresponding docs for this guide are available on our [docs](https://docs.teleflow.khulnasoft.com/channels-and-providers/chat/discord).
<Note>The entire code for this app is available on our [GitHub](https://github.com/khulnasoft/discord-chat-app).</Note>
...
# Set up Teleflow
In this part, we'll set up a workflow that will be triggered when we send a notification. Workflows are like blueprints and hold the entire flow of notifications. You can read more about them on our [docs](https://docs.teleflow.khulnasoft.com/workflows/notification-workflows). To set up a notification workflow for our app, follow these steps:
1. Make sure that you've set the Discord Integration as active from the [Teleflow Integrations Store](https://web.teleflow.khulnasoft.com/integrations?utm_campaign=docs-discordnotifications).
<Frame caption="Make sure Discord Integration is active"><img src="/images/discord-guide/active.png" /> </Frame>
2. Goto the [Teleflow Web Dashboard](https://web.teleflow.khulnasoft.com/workflows?utm_campaign=docs-discordnotifications).
...
export const chat = async (chatMsg) => {
    try {
        ...
        await teleflow.subscribers.setCredentials(process.env.SUB_ID, ChatProviderIdEnum.Discord, {
...
teleflow.trigger('chat-with-discord', {
    to: {
...
export const chat = async (chatMsg) => {
    try {
        ...
        await teleflow.trigger('chat-with-discord', {
            to: {
...
# Front end set up
For our demonstration purposes, the front-end is pretty basic. All we have to do is have an input field to take the notification text in and send the payload. The front-end code for this demo app is available on our [Github repo](https://github.com/khulnasoft/discord-chat-app/tree/main/frontend). 
And here's our app in all its glory!

<Frame caption="Our app in action!"><img src="/images/discord-guide/outputt.gif" /> </Frame>

This is how we send Discord notifications using Teleflow!