...
You can find the entire code(frontend as well as backend) for this app [here](https://github.com/khulnasoft/digest-learning-app/tree/main).
...
- A Teleflow account. [Sign up for free](http://web.teleflow.khulnasoft.com/?utm_campaign=docs-gs-digestsinapp) if you don’t have one yet.
...
```jsx
import { sendDigest } from '../teleflow/teleflow.js';
...
To make it modular, we’ll keep the trigger code in a separate function in a separate file (`teleflow.js`, in our case) and the trigger function is getting called in the controller function above by the name `getDigest`.
...
Subscribers are entities to which the notifications are sent. You can see a list of subscribers in the [Teleflow dashboard](https://web.teleflow.khulnasoft.com/subscribers?utm_campaign=docs-digests-inapp) as well.
...
```jsx
await teleflow.subscribers.identify('aa234u787', {
...
export const sendDigest = async (name) => {
  ...
  await teleflow.trigger('digest-showcase', {
    to: {
...
Now, we just need to hit the route defined above from the front end and add Teleflow to it.

# Add Teleflow’s Notification Center to the front end

In the front end, install the Teleflow notification centre package using the following command:
...
window.location.href = message.cta.data.url;
  ...
  return (
    <TeleflowProvider subscriberId={'USER_ID'} applicationIdentifier={'APP_ID_FROM_ADMIN_PANEL'}>
  ...
  return (
    <div className="bell">
      <div className="intro">
        ...
        <p className="desc">
          Don't know how to? Start <a href="https://docs.teleflow.khulnasoft.com/platform/digest/">here</a>.
        </p>
      </div>
...
You’ll also need to plug in your `applicationIdentifier`. You can get your `application id` from the settings menu in the Teleflow dashboard.
...
Congratulations for following the guide up until this point. We can style our app a little using [some CSS](https://github.com/khulnasoft/digest-learning-app/blob/main/frontend/src/App.css).
