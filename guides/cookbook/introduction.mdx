...
This cookbook contains recipes and code samples demonstrating how to accomplish everyday tasks with Teleflow in your application. Each code example uses our libraries and SDKs.

<Frame caption="Workflow, Feed, Messages & Notifications Flow in Teleflow">
...
<Warning>**Subscriber Feed** is very different from **Activity Feed**. The former is for In-App channels, while the latter is a list of every message and relevant metadata across all channels shown in your dashboard.</Warning>

The code sample below fetches the list of all In-App messages sent to a specific subscriber:

<CodeGroup>
```javascript Node.js 
const { data: inAppMessages } = await teleflow.subscribers.getNotificationsFeed('subscriberId', {
  page: 0,
  limit: 10,
...
You can send notifications to Slack Channels via Teleflow like so:
...
// Identify Subscriber
await teleflow.subscribers.identify('<SUBSCRIBER_ID>', {
  firstName: 'newSubForSlackChat',
});

// Set credentials for the Subscriber
await teleflow.subscribers.setCredentials('<SUBSCRIBER_ID>', ChatProviderIdEnum.Slack, {
...
Follow the [full guide](/guides/slack-guide) on how to send Slack notifications using Teleflow.