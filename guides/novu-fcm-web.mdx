---
title: 'How to send web push notifications with FCM and Teleflow'
description: 'Send FCM push notifications in a React app using Teleflow'
---

## Introduction
In this guide, we'll learn how to send FCM push notifications in a web app using Teleflow. But before exploring the actual code, let’s understand what a push notification is and how it works.
You can find the [frontend code](https://github.com/khulnasoft/fcmWebPushFE) as well as [backend code](https://github.com/khulnasoft/FCMWebPushBE) for the app on Github.
<br/>
Push notifications are notifications that are sent to a user's devices whether the user is using the app or not. We'll be using Firebase Cloud Messaging (FCM) integration of Teleflow to send these notifications. This guide will be broken down into three parts:
1. Setting up Teleflow.
2. Setting up Firebase.
3. Adding Firebase to the Frontend.
4. Adding Teleflow to the Backend.
...
5. We're done with the Firebase setup! Now, we'll set up Teleflow and finally proceed to add both (Teleflow and FCM) to our app.
...
1. Head over to the [Integrations Store](https://web.teleflow.khulnasoft.com/integrations?utm_campaign=docs-guides-fcm-web) on the Teleflow Web Dashboard and make sure that you've turned on the 'Firebase Cloud Messaging' integration.
...
2. Now, we need to create a workflow that we'll trigger from our app to send notifications. Head over to [workflows in the Teleflow Web Dashboard](https://web.teleflow.khulnasoft.com/workflows?utm_campaign=docs-guides-fcm-web) and click on the 'Add a workflow' button.
...
## Adding Teleflow to the backend
In your app's backend, add Teleflow using the following command:
```bash
...
```jsx
import { inAppNotification } from "../teleflow/teleflow.js";
...
To make it modular, we’ll keep the trigger code in a separate function in a separate file (`teleflow.js`, in our case) and the trigger function is getting called in the controller function above by the name `createNotif`.
...
[Subscribers](https://docs.teleflow.khulnasoft.com/subscribers/subscribers) are entities to which the notifications are sent. You can see a list of subscribers in the [Teleflow dashboard](https://web.teleflow.khulnasoft.com/subscribers?utm_campaign=docs-guides-fcm-web) as well.
...
```jsx
 await teleflow.subscribers.identify(process.env.TELEFLOW_SUB_ID, {
        firstName: "pushSubscriber"
    });
```
Here, we're creating a subscriber with the `subscriberID` of whatever value the `env` file contains for the identifier `TELEFLOW_SUB_ID`. You can read more about subscribers [in our docs.](/subscribers/subscribers).

Back in our app, before we can now add the trigger function, we need to set device identifiers using the [setCredential method](https://docs.teleflow.khulnasoft.com/channels-and-providers/push/fcm#set-device-token):
```jsx
await teleflow.subscribers.setCredentials(process.env.TELEFLOW_SUB_ID, PushProviderIdEnum.FCM, {
...
export const inAppNotification = async (title, body) => {
    ...
    teleflow.trigger(process.env.TELEFLOW_WORKFLOW_ID, {
        to: {
            subscriberId: process.env.TELEFLOW_SUB_ID
        },
        payload: {
            title: title,
            body: body
...
We're done with the app. You can find the [frontend code](https://github.com/khulnasoft/fcmWebPushFE) as well as [backend code](https://github.com/khulnasoft/FCMWebPushBE) for the app on Github.

Congratulations on following the guide up until this point. If you’ve done everything as recommended, you’ll end up with an app that uses Firebase Cloud Messaging to send notifications to your users using Teleflow.