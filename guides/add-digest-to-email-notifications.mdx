...
You can find the entire code(frontend as well as backend) for this app [here](https://github.com/khulnasoft/digest-email-app).
...
Teleflow has a built-in digest engine that collects multiple trigger events, aggregates them into a single message, and delivers it to the subscriber. You can use the digest engine by adding a ‘digest node’ to your workflow in the workflow editor in the [Teleflow dashboard](https://web.teleflow.khulnasoft.com/workflows?utm_campaign=docs-digests). If you want to learn more about it, [this](/getting-started/concepts#digest) is a great place to start.
...
- A Teleflow account. [Sign up for free](http://web.teleflow.khulnasoft.com/?utm_campaign=docs-gs-digests) if you don’t have one yet.
- A working React development environment.

# Workflow setup in Teleflow

Once, you have these, follow the steps below:

1. Head over to the Teleflow Dashboard.
2. Click `Workflows` on the left sidebar of your Teleflow dashboard.
...
Now, let’s see how to add Teleflow to our app!

# Add Teleflow to the Backend and Configure it

In your backend, install the teleflow package using the following code:
...
```jsx
import { sendEmailDigest } from '../teleflow/teleflow.js';
...
To make it modular, we’ll keep the trigger code in a separate function in a separate file (`teleflow.js`, in our case) and the trigger function is getting called in the controller function above by the name `getEmailDigest`.
...
Subscribers are users to which the notifications are sent. You can see a list of subscribers in the [Teleflow dashboard](https://web.teleflow.khulnasoft.com/subscribers?utm_campaign=docs-digests) as well.

In our app, we’ll create a subscriber in Node.js as we’re writing our backend in Node.js, but we also have backend [SDKs](/sdks/introduction) ([Node.js](https://github.com/khulnasoft/teleflow/tree/next/packages/node), [PHP](https://github.com/khulnasoft/teleflow-php), [.NET](https://github.com/khulnasoft/teleflow-dotnet), [Elixir](https://github.com/khulnasoft/teleflow-elixir), [Go](https://github.com/khulnasoft/go-teleflow), [Ruby](https://github.com/khulnasoft/teleflow-ruby), [Python](https://github.com/khulnasoft/teleflow-python), and [Kotlin](https://github.com/khulnasoft/teleflow-kotlin)) to choose from. The recommended way to create a subscriber in NodeJS is as follows:

```jsx
await teleflow.subscribers.identify('digestEmailSub', {
  firstName: 'digest email subscriber',
  email: email,
});
```

Here, we’re creating a subscriber with the `subscriberID` of `digestEmailSub.` In most real-world scenarios, the `subscriberId` should be an alphanumeric entity like `adfa67y87ad0`. You can read more about subscribers [here](https://docs.teleflow.khulnasoft.com/platform/subscribers/).
...
export const sendEmailDigest = async (notif, email) => {
  const teleflow = new Teleflow(process.env.YOUR_TELEFLOW_API_KEY);
  await teleflow.subscribers.identify('digestEmailSub', {
    firstName: 'digest email subscriber',
    email: email,
  });

  teleflow.trigger('emaildigestworkflow', {
    to: {
  ...
  return (
    <div>
      ...
      <p>
        ...
        <a className="underline" href="https://docs.teleflow.khulnasoft.com/platform/digest/">
          {' '}
          here
        </a>
      </p>
...
Congratulations on following the guide up until this point. We can style our app a little using [some TailwindCSS](https://github.com/khulnasoft/digest-email-app/blob/main/frontend/src/app.css).
