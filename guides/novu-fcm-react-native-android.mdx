---
title: 'How to send push notifications in an Android app (react native) with FCM and Teleflow'
description: 'Learn how to send FCM push notifications in a React native Android app using Teleflow and FCM'
---
...
- A [Teleflow](https://web.teleflow.khulnasoft.com/?utm_campaign=docs-gs-guides-fcm-android) account
- An [Expo](https://expo.dev) account

Push notifications are notifications that are sent to a user’s devices whether the user is using the app or not. We’ll be using the Firebase Cloud Messaging (FCM) integration of Teleflow to send these notifications. You can find the complete code for this project in our [Github repo.](https://github.com/khulnasoft/fcm-react-native-android) 

## Start a React-Native project
We'll use the `create-expo` tool to initialize a new expo app. It creates a new React Native app with the expo package installed. We're naming our app `fcm-teleflow-rn2` and hence using the following command:
``` javascript
 npx create-expo-app fcm-teleflow-rn2
```
...
<Frame caption="Now, we need to add firebase to our app"><img src="/images/fcm-teleflow-rna/greet.png" /> </Frame>
3. We'll choose the 'Android' option because we're creating an Android app.
<Frame caption="Choose the web option to connect to a web app"><img src="/images/fcm-teleflow-rna/android.png" /> </Frame>
4. Find the package name from the `app.json` file of the project:
<Frame caption="Find the package name from the `app.json` file."><img src="/images/fcm-teleflow-rna/json.png" /> </Frame>
5. Add this package name to Firebase:
<Frame caption="Add package name to Firebase."><img src="/images/fcm-teleflow-rna/package.png" /> </Frame>
...
<Frame caption="Run the given command to obtain `SHA1` key."><img src="/images/fcm-teleflow-rna/keys.png" /> </Frame>
7. Now copy the `SHA1` key and add it to Firebase and register the app:
<Frame caption="Add the `SHA1` key to the Firebase app."><img src="/images/fcm-teleflow-rna/key.png" /> </Frame>
8. Click 'next' on all the other options and finally click 'continue to console' as follows:
<Frame caption="We'll add the second `SHA1` key now."><img src="/images/fcm-teleflow-rna/console.png" /> </Frame>
9. Go to 'project settings' and scroll all the way down
<Frame caption="Scroll till you see the `SHA1` key you added earlier."><img src="/images/fcm-teleflow-rna/settings.png" /> </Frame>
10. Click on the 'Add fingerprint' button and add the second `SHA1` key here.
<Frame caption="Add the second `SHA1` key."><img src="/images/fcm-teleflow-rna/fp.png" /> </Frame>
...
<Frame caption="Specify the location of Google services file in the `app.json` file."><img src="/images/fcm-teleflow-rna/location.png" /> </Frame>
...
<Frame caption="Find the server key in the project settings."><img src="/images/fcm-teleflow-rna/server.png" /> </Frame>
...
<Frame caption="Start the server to run the app."><img src="/images/fcm-teleflow-rna/qr.png" /> </Frame>
...
1. Head over to the [Integrations Store](https://web.teleflow.khulnasoft.com/integrations?utm_campaign=docs-guides-fcm-android) on the Teleflow Web Dashboard and make sure that you've turned on the 'Firebase Cloud Messaging' integration.
<Frame caption="Turn on Firebase Cloud Messaging integration from the Teleflow Dashboard"><img src="/images/fcm-web-push/teleflow-dashboard.png" /> </Frame>
<Info>If you're doing this for the first time, you'll need to get your service account key from the [Firebase Console](https://console.firebase.google.com/). Read more about it in our [FCM provider docs.](https://docs.teleflow.khulnasoft.com/channels-and-providers/push/fcm)</Info>
2. Now, we need to create a workflow that we'll trigger from our app to send notifications. Head over to [workflows in the Teleflow Web Dashboard](https://web.teleflow.khulnasoft.com/workflows?utm_campaign=docs-guides-fcm-android) and click on the 'Add a workflow' button.
...
<Frame caption="Add the identifiers that you'll use"><img src="/images/fcm-teleflow-rna/title.png" /> </Frame>
6. Now, click on the 'Update' and then the 'get snippet' button to get the trigger code.
<Frame caption="Click on the 'get snippet' button to get the trigger code"><img src="/images/fcm-teleflow-rna/snippet.png" /> </Frame>

## Create a subscriber and update credentials
A subscriber is essentially the recipient of the notifications sent through Teleflow’s system. When you create a subscriber, you’re setting up the necessary information for Teleflow to send targeted notifications to. You can find more about subscribers in [our docs.](https://docs.teleflow.khulnasoft.com/subscribers/subscribers)
To create a subscriber, we'll use the `identify` method:
``` jsx
// creating new instance of Teleflow
const teleflow = new Teleflow("<Teleflow API key>");

// creating subscriber
await teleflow.subscribers.identify('newSubForRNA', {
    firstName: "RNA"
});
```
...
``` jsx
await teleflow.subscribers.setCredentials('newSubForRNA', PushProviderIdEnum.FCM, {
...
## Sending a push notification using Teleflow

To send a notification with Teleflow, we are actually triggering a notification workflow. Here's how to do it using Node:
``` jsx
teleflow.trigger('rna-2', {
    to: {
...
<Frame caption="A notification is successfully received on the device"><img src="/images/fcm-teleflow-rna/hardcoded.png" /> </Frame>
...
<Frame caption="You can send dynamic content in notifications by changing the workflow."><img src="/images/fcm-teleflow-rna/custom.png" /> </Frame>
...
<Frame caption="A notification with dynamic content is successfully received on the device"><img src="/images/fcm-teleflow-rna/dynamic.png" /> </Frame>
...
<Frame caption="A notification with image content is successfully received on the device"><img src="/images/fcm-teleflow-rna/image.png" /> </Frame>
...
<Frame caption="A notification with custom data content is successfully received on the device"><img src="/images/fcm-teleflow-rna/cd.png" /> </Frame>
...
<Frame caption="A low priority notification is successfully received on the device"><img src="/images/fcm-teleflow-rna/priority.png" /> </Frame>
