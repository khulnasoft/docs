...
Inputs and Payload are defined using [JSON Schema](/echo/recipes/json-schema), providing a strong run-time validation system for your workflows.
...
- **Step Inputs** - Managed in the Teleflow Web UI, and are static
- **Payload Schema** - Passed during the `teleflow.trigger` method, and are dynamic values for each trigger

## Input Schema
[Step](/echo/concepts/steps) input schema is defining the input passed during the `step` method. Those inputs can be modified and persisted in the Teleflow Web UI.
...
Payload schema is defining the payload passed during the `teleflow.trigger` method. This is useful for ensuring that the payload is correctly formatted and that the data is valid.
...
```tsx
teleflow.trigger('comment-on-post', {
    to: 'subscriber_id',
    payload: {
        post_id: 1234,
        comment: 'Looks good!'
