---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/health/REFERENCE.md"
...
docs_link: "https://docs.khulnasoft.com/docs/alerts-&-notifications/alert-configuration-reference"
---

# Configure alerts

Khulnasoft's health watchdog is highly configurable, with support for dynamic thresholds, hysteresis, alert templates, and
more. You can tweak any of the existing alerts based on your infrastructure's topology or specific monitoring needs, or
create new entities.

You can use health alerts in conjunction with any of Khulnasoft's [collectors](/docs/collecting-metrics) (see
the [supported collector list](/docs/collecting-metrics/monitor-anything)) to monitor the health of your systems, containers, and
applications in real time.

While you can see active alerts both on the local dashboard and Khulnasoft Cloud, all health alerts are configured _per
node_ via individual Khulnasoft Agents. If you want to deploy a new alert across your
[infrastructure](/docs/khulnasoft-cloud/spaces-and-rooms), you must configure each node with the same health configuration
files.

## Reload health configuration

You don’t need to restart the Khulnasoft Agent between changes to health configuration files, such as specific health entities. Instead, you can use `khulnasoftcli` and the `reload-health` option to prevent gaps in metrics collection.

```bash
sudo khulnasoftcli reload-health
```

If `khulnasoftcli` doesn't work on your system, send a `SIGUSR2` signal to the daemon, which reloads health configuration without restarting the entire process.

```bash
killall -USR2 khulnasoft
```
...
- The `[health]` section in `khulnasoft.conf`. By editing the daemon's behavior, you can disable health monitoring
    altogether, run health checks more or less often, and more. See
    [daemon configuration](/docs/khulnasoft-agent/configuration/daemon#health-section-options) for a table of
...
Navigate to your [Khulnasoft config directory](/docs/khulnasoft-agent/configuration) and
...
Save the file and [reload Khulnasoft's health configuration](#reload-health-configuration) to apply your changes.
...
[health management API](/docs/alerts-&-notifications/health-api-calls). The
available options are described below.

### Disable all alerts

In the `khulnasoft.conf` `[health]` section, set `enabled` to `no`, and restart the Agent.

### Disable some alerts

In the `khulnasoft.conf` `[health]` section, set `enabled alarms` to a
[simple pattern](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns) that
...
and [reload Khulnasoft's health configuration](#reload-health-configuration).
...
This action requires that you [reload Khulnasoft's health configuration](#reload-health-configuration).
...
[health management API](/docs/alerts-&-notifications/health-api-calls).
...
To write a new health entity into a new file, navigate to your [Khulnasoft config directory](/docs/khulnasoft-agent/configuration),
...
- `warn`/`crit`: The value at which Khulnasoft should trigger a warning or critical alert. This example uses simple
...
When you finish writing this new health entity, [reload Khulnasoft's health configuration](#reload-health-configuration) to
see it live on the local dashboard or Khulnasoft Cloud.

## Health entity reference

The following reference contains information about the syntax and options of _health entities_, which Khulnasoft attaches
...
If the `alert` and `template` entities have the same name and are attached to the same chart, Khulnasoft will use `alarm`.

### Entity format

Khulnasoft parses the following lines. Beneath the table is an in-depth explanation of each line's purpose and syntax.
...
- A few lines use space-separated lists to define how the entity behaves. You can use `*` as a wildcard or prefix with
    `!` for a negative match. Order is important, too! See our [simple patterns docs](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns) for
    more examples.
...
<details>
<summary>Khulnasoft's stock alerts use the following `class` attributes by default:</summary>
...
<details>
<summary>Khulnasoft's stock alerts use the following `type` attributes by default, but feel free to adjust for your own requirements.</summary>
...
| Khulnasoft         | Internal Khulnasoft components monitoring                                                         |
...
The full [database query API](/docs/developer-and-contributor-corner/rest-api/queries) is supported. In short:
...
- `of DIMENSIONS` is optional and has to be the last parameter. Dimensions have to be separated
     ...
     have spaces in their names). This accepts Khulnasoft simple patterns _(with `words` separated by
...
When paired with `lookup`, `calc` will perform the calculation just after `lookup` has retrieved a value from Khulnasoft's
database.
...
The format uses Khulnasoft's [expression syntax](#expressions).
...
The default `SCRIPT` is Khulnasoft's `alarm-notify.sh`, which supports all the notifications methods Khulnasoft supports,
...
default interval settings inherited from health settings in `khulnasoft.conf`. The default settings for repeating
notifications are `default repeat warning = DURATION` and `default repeat critical = DURATION` which can be found in
health stock configuration, when one of these intervals is bigger than 0, Khulnasoft will activate the repeat notification
...
good idea to tell Khulnasoft to not clear the notification, by using the `no-clear-notification` option.

#### Alert line `host labels`

Defines the list of labels present on a host. See our [host labels guide](/docs/khulnasoft-agent/configuration/organize-systems-metrics-and-alerts) for
an explanation of host labels and how to implement them.

For example, let's suppose that `khulnasoft.conf` is configured with the following labels:
...
And more labels in `khulnasoft.conf` for workstations:
...
By defining labels inside of `khulnasoft.conf`, you can now apply labels to alerts. For example, you can add the following
...
See our [simple patterns docs](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns) for more examples.
...
An alert acting on a chart that has a chart label named e.g. `target`, with a value of `https://khulnasoft.com/`,
...
```yaml
info: average ratio of HTTP responses with unexpected status over the last 5 minutes for the site https://khulnasoft.com/
```
...
Khulnasoft has an internal infix expression parser under `libkhulnasoft/eval`. This parses expressions and creates an internal
...
specified. The format is: `(condition) ? (true expression) : (false expression)`. So, Khulnasoft will first evaluate the
...
registry](https://registry.my-khulnasoft.io/api/v1/alarm_variables?chart=system.cpu).

<!-- > If you don't know how to find the CHART_NAME, you can read about it [here](https://github.com/khulnasoft/khulnasoft/blob/master/src/web/README.md#charts). -->

Khulnasoft supports three internal indexes for variables that will be used in health monitoring.
...
- `REMOVED` - the alert has been deleted (this happens when a SIGUSR2 is sent to Khulnasoft
...
Check the `health/health.d/` directory for all alerts shipped with Khulnasoft.
...
The above checks that Khulnasoft is able to collect data from apache. In detail:
...
When Khulnasoft detects a dropped packet, it will add the chart, and it will automatically attach this
alert to it.
...
### Example 6 - [Anomaly rate](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection#anomaly-rate) based CPU chart alert

Warning if the 5-minute rolling [anomaly rate](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection#anomaly-rate) averaged across all CPU dimensions is above 5%, critical if it goes above 20%:
...
Khulnasoft will create one alert for the chart.

### Example 7 - [Anomaly rate](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection#anomaly-rate) based node level alert

Warning if the 5-minute rolling [anomaly rate](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection#anomaly-rate) averaged across all ML enabled dimensions is above 5%, critical if it goes above 20%:
...
The `lookup` line will use the `anomaly_rate` dimension of the `anomaly_detection.anomaly_rate` ML chart to calculate the average [node level anomaly rate](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection#anomaly-rate) over the last 5 minutes.

## Troubleshooting

You can compile Khulnasoft with [debugging](/docs/khulnasoft-agent/daemon#debugging) and then set in `khulnasoft.conf`:
...
Then check your `/var/log/khulnasoft/debug.log`. It will show you how it works. Important: this will generate a lot of
output in debug.log.

You can find the context of charts by looking up the chart in either `http://NODE:19999/khulnasoft.conf` or
`http://NODE:19999/api/v1/charts`, replacing `NODE` with the IP address or hostname for your Agent dashboard.

You can find how Khulnasoft interpreted the expressions by examining the alert at
`http://NODE:19999/api/v1/alarms?all`. For each expression, Khulnasoft will return the expression as given in its
