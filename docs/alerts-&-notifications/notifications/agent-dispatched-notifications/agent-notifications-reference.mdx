---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/health/notifications/README.md"
...
sidebar_position: "1202500"
docs_link: "https://docs.khulnasoft.com/docs/alerts-&-notifications/notifications/agent-dispatched-notifications/agent-notifications-reference"
---

# Agent alert notifications

This is reference documentation for Khulnasoft's Agent alert notification feature, which supports dozens of endpoints, user roles, and more.

The `script to execute on alarm` line in `khulnasoft.conf` defines the external script that will be called once the alert is triggered.
...
>
> - To edit configuration files safely, we provide the [`edit config` script](/docs/khulnasoft-agent/configuration#edit-a-configuration-file-using-edit-config)located in your [Khulnasoft config directory](/docs/khulnasoft-agent/configuration#the-khulnasoft-config-directory) (typically is `/etc/khulnasoft`) that creates the proper file and opens it in an editor automatically.  
> Note that to run the script you need to be inside your Khulnasoft config directory.
>
> - Please also note that after most configuration changes, you will need to [restart the Agent](/docs/khulnasoft-agent/maintenance/service-control) for the changes to take effect.
>
> It is recommended to use this way for configuring Khulnasoft.

You can change the default script globally by editing `khulnasoft.conf` and changing the `script to execute on alarm` in the `[health]` section.
...
If you are [running your own Registry](/docs/khulnasoft-agent/configuration/registry#configure-a-custom-registry), add `export KHULNASOFT_REGISTRY_URL=[YOUR_URL]` before calling `alarm-notify.sh`.
...
>
>```sh
>bash -x /usr/libexec/khulnasoft/plugins.d/alarm-notify.sh test
>```
>
...
To allow notification providers to fetch the icons/images, by default we set the URL of the global public Khulnasoft Registry.

If you have an Internet facing khulnasoft (or you have copied the images/ folder
of Khulnasoft to your web server), set its URL here, to fetch the notification
images from it.

```text
images_base_url="http://my.public.khulnasoft.server:19999"
```

### Date handling

You can configure khulnasoft alerts to send dates in any format you want via editing the `date_format` variable.
...
By default, Khulnasoft will use the simple hostname for the system (the hostname with everything after the first `.` removed) when displaying the hostname in alert notifications.

If you instead prefer to have Khulnasoft use the host's fully qualified domain name, you can set `use_fdqn` to `YES`.

This setting does not account for child systems for which the system you are configuring is a parent.

> ### Note
>
> If the system's host name is overridden in `/etc/khulnasoft.conf` with the `hostname` option, that name will be used unconditionally.
