---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/web/api/health/README.md"
sidebar_label: "Health API Calls"
docs_status: "Published"
docs_rel_path: "Alerts & Notifications"
sidebar_position: "1204010"
docs_link: "https://docs.khulnasoft.com/docs/alerts-&-notifications/health-api-calls"
---
...
Khulnasoft enables alerts on demand, i.e. when the chart they should be linked to starts collecting data. So, although many
...
The size of the alert log is configured in `khulnasoft.conf`. There are 2 settings: the event history kept in the DB (in seconds), and the in memory size of the alert log.
...
Khulnasoft v1.12 and beyond provides a command API to control health checks and notifications at runtime. The feature is especially useful for maintenance periods, during which you receive meaningless alerts.
From Khulnasoft v1.16.0 and beyond, the configuration controlled via the API commands is [persisted across Khulnasoft restarts](#persistence).
...
-   Silence alert notifications. Alert conditions will be evaluated, the alerts will appear in the log and the Khulnasoft UI will show the alerts as active, but no notifications will be sent.
-   Disable or Silence specific alerts that match selectors on alert/template name, chart, context, and host.

The API is available by default, but it is protected by an `api authorization token` that is stored in the file you will see in the following entry of `http://NODE:19999/khulnasoft.conf`:

```
[registry]
    # khulnasoft management api key file = /var/lib/khulnasoft/khulnasoft.api.key
```
...
By default access to the health management API is only allowed from `localhost`. Accessing the API from anything else will return a 403 error with the message `You are not allowed to access this resource.`. You can change permissions by editing the `allow management from` variable in `khulnasoft.conf` within the [web] section. See [web server access lists](/docs/khulnasoft-agent/configuration/securing-agents/web-server-reference#access-lists) for more information.

The command `RESET` just returns Khulnasoft to the default operation, with all health checks and notifications enabled.
...
Alerts may then still be raised and logged in Khulnasoft, so you'll be able to see them via the UI.  
...
The `selection criteria` are key/value pairs, in the format `key : value`, where value is a Khulnasoft [simple pattern](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns). This means that you can create very powerful selectors (you will rarely need more than one or two).
...
The command `LIST` was added in Khulnasoft v1.16.0 and returns a JSON with the current status of the silencers.
...
From Khulnasoft v1.16.0 and beyond, the silencers configuration is persisted to disk and loaded when Khulnasoft starts.
...
The file's location is configurable in `khulnasoft.conf`. The default is shown below:

```
[health]
        # silencers file = /var/lib/khulnasoft/health.silencers.json
```

### Further reading

The test script under [tests/health_mgmtapi](/docs/developer-and-contributor-corner/health-command-api-tester) contains a series of tests that you can either run or read through to understand the various calls and responses better.
