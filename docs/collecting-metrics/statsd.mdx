---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/collectors/statsd.plugin/README.md"
sidebar_label: "StatsD"
docs_status: "Published"
toc_max_heading_level: "4"
docs_rel_path: "Collecting Metrics"
sidebar_position: "700030"
docs_link: "https://docs.khulnasoft.com/docs/collecting-metrics/statsd"
---
...
If you want to docs more about the StatsD protocol, we have written a 
[blog post](https://blog.khulnasoft.com/introduction-to-statsd/) about it!


Khulnasoft is a fully featured statsd server. It can collect statsd formatted metrics, visualize 
them on its dashboards and store them in it's database for long-term retention.

Khulnasoft statsd is inside Khulnasoft (an internal plugin, running inside the Khulnasoft daemon), it is 
configured via `khulnasoft.conf` and by-default listens on standard statsd port 8125. Khulnasoft supports 
both TCP and UDP packets at the same time. 

Since statsd is embedded in Khulnasoft, it means you now have a statsd server embedded on all your servers. 

Khulnasoft statsd is fast. It can collect several millions of metrics per second on modern hardware, using 
...
Khulnasoft ships with a few synthetic chart definitions to automatically present application metrics into a 
...
- [K6 load testing tool](https://k6.io)
  ...
  - [Documentation](https://github.com/khulnasoft/khulnasoft/blob/master/src/collectors/statsd.plugin/k6.md)
  - [Configuration](https://github.com/khulnasoft/khulnasoft/blob/master/src/collectors/statsd.plugin/k6.conf)
- [Asterisk](https://www.asterisk.org/)
  - **Description:** Asterisk is an Open Source PBX and telephony toolkit.
  - [Documentation](https://github.com/khulnasoft/khulnasoft/blob/master/src/collectors/statsd.plugin/asterisk.md)
  - [Configuration](https://github.com/khulnasoft/khulnasoft/blob/master/src/collectors/statsd.plugin/asterisk.conf)

## Metrics supported by Khulnasoft

Khulnasoft fully supports the StatsD protocol and also extends it to support more advanced Khulnasoft specific use cases. 
All StatsD client libraries can be used with Khulnasoft too.
...
- **Sets**
     ...
     When a set is not collected, Khulnasoft **defaults** to showing a zero value, until a data collection event changes the value.
...
Currently, Khulnasoft uses only 2 tags:
...
Charts are not updated to change units or dimension names once they are created. So, either send the tags on every event, or use the special `zinit` value to initiaze the charts at the beginning. `zinit` is a special value that can be used on any chart, to have khulnasoft initialize the charts, without actually setting any values to them. So, instead of sending `my.metric:VALUE|c|#units=bytes,name=size` every time, the application can send at the beginning `my.metric:zinit|c|#units=bytes,name=size` and then `my.metric:VALUE|c`.

#### Overlapping metrics

Khulnasoft's StatsD server maintains different indexes for each of the metric types supported. This means the same metric `name` may exist under different types concurrently.

#### How to name your metrics

A good practice is to name your metrics like `application.operation.metric`, where:

- `application` is the application name - Khulnasoft will automatically create a dashboard section based on the first keyword of the metrics, so you can have all your applications in different sections.
- `operation` is the operation your application is executing, like `dbquery`, `request`, `response`, etc.
- `metric` is anything you want to name your metric as. Khulnasoft will automatically append the metric type (meter, counter, gauge, set, dictionary, timer, histogram) to the generated chart.
...
Khulnasoft accepts multiple metrics per packet if each is terminated with a newline (`\n`) at the end.

#### TCP packets

Khulnasoft listens for both TCP and UDP packets. For TCP, is it important to always append `\n` on each metric, as Khulnasoft will use the newline character to detect if a metric is split into multiple TCP packets. 
...
Khulnasoft will accept UDP packets up to 9000 bytes, but the underlying network will not exceed MTU. 

> You can read more about the network maximum transmission unit(MTU) in this cloudflare [article](https://www.cloudflare.com/en-gb/docsing/network-layer/what-is-mtu/).

## Configuration

You can find the configuration at `/etc/khulnasoft/khulnasoft.conf`:
...
-   `enabled = yes|no`
     ...
     controls if StatsD will be enabled for this Khulnasoft. The default is enabled.
...
-   `update every (flushInterval) = 1s` controls the frequency StatsD will push the collected metrics to Khulnasoft charts.

-   `decimal detail = 1000` controls the number of fractional digits in gauges and histograms. Khulnasoft collects metrics using signed 64-bit integers and their fractional detail is controlled using multipliers and divisors. This setting is used to multiply all collected values to convert them to integers and is also set as the divisors, so that the final data will be a floating point number with this fractional detail (1000 = X.0 - X.999, 10000 = X.0 - X.9999, etc).

The rest of the settings are discussed below.

## StatsD charts

Khulnasoft can visualize StatsD collected metrics in 2 ways:
...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns). Khulnasoft will create private charts for all metrics **by default**.
...
You can specify Khulnasoft StatsD to have a different `memory mode` than the rest of the Khulnasoft Agent. You can read more about `memory mode` in the [documentation](/docs/khulnasoft-agent/database).

The default behavior is to use the same settings as the rest of the Khulnasoft Agent. If you wish to change them, edit the following settings:
- `private charts memory mode`
- `private charts history`

### Optimize private metric charts storage

For optimization reasons, Khulnasoft imposes a hard limit on private metric charts. The limit is set via the `max private charts hard limit` setting (which defaults to 1000 charts). Metrics above this hard limit are still collected, but they can only be used in synthetic charts (once a metric is added to chart, it will be sent to backend servers too).

If you have many ephemeral metrics collected (i.e. that you collect values for a certain amount of time), you can set the configuration option `set charts as obsolete after`. Setting a value in seconds here, means that Khulnasoft will mark those metrics (and their private charts) as obsolete after the specified time has passed since the last sent metric value. Those charts will later be deleted according to the setting in `cleanup obsolete charts after`. Setting `set charts as obsolete after` to 0 (which is also the default value) will disable this functionality.
...
-   **application** aka section in Khulnasoft Dashboard.
-   **charts for each application** aka family in Khulnasoft Dashboard.
-   **StatsD metrics for each chart** /aka charts and context Khulnasoft Dashboard.

> You can read more about how the Khulnasoft Agent organizes information in the relevant [documentation](https://github.com/khulnasoft/khulnasoft/blob/master/src/web/README.md)

For each application you need to create a `.conf` file in `/etc/khulnasoft/statsd.d`.

For example, if you want to monitor the application `myapp` using StatsD and Khulnasoft, create the file `/etc/khulnasoft/statsd.d/myapp.conf`, with this content:
```
[app]
	name = myapp
	metrics = myapp.*
...
-   `metrics` is a Khulnasoft [simple pattern](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
-   `memory mode` sets the memory mode for all charts of the application. The default is the global default for Khulnasoft (not the global default for StatsD private charts). We suggest not to use this (we have commented it out in the example) and let your app use the global default for Khulnasoft, which is our dbengine.

-   `history` sets the size of the round-robin database for this application. The default is the global default for Khulnasoft (not the global default for StatsD private charts). This is only relevant if you use `memory mode = save`. Read more on our documentation for the Agent's [Database](/docs/khulnasoft-agent/database).
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Khulnasoft [simple pattern](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
Contents of file `/etc/khulnasoft/statsd.d/foobar.conf`:
...
You can define a dictionary to rename metrics sent by StatsD clients. This enables you to send response `"200"` and Khulnasoft visualize it as `succesful connection`

The `[dictionary]` section accepts any number of `name = value` pairs.

Khulnasoft uses this dictionary as follows:
...
3.  If any of the above succeeds, Khulnasoft uses the `value` of the dictionary, to set the name of the dimension. The dimensions will have as ID the original StatsD metric name, and as name, the dictionary value.
...
> keep in mind that if you add multiple times the same StatsD metric to a chart, Khulnasoft will append `TYPE` to the dimension ID, so `myapp.metric1` will be added as `myapp.metric1_last` or `myapp.metric1_events`, etc. If you add multiple times the same metric with the same `TYPE` to a chart, Khulnasoft will also append an incremental counter to the dimension ID, i.e. `myapp.metric1_last1`, `myapp.metric1_last2`, etc.

#### Dimension patterns

Khulnasoft allows adding multiple dimensions to a chart, by matching the StatsD metrics with a Khulnasoft simple pattern.
...
The above will add dimension named `200`, `400` and `500`. Khulnasoft extracts the wildcard part of the metric name - so the dimensions will be named with whatever the `*` matched. 
...
Getting the proper support for a programming language is not always easy, but the Unix shell is available on most Unix systems. You can use shell and `nc` to instrument your systems and send metric data to Khulnasoft's StatsD implementation.
...
`-N` tells `nc` to close the socket once it receives EOF on its input. `-w 120` tells `nc` to stop if the connection is idle for 120 seconds. The timeout is needed to stop the `nc` command if you restart Khulnasoft while `nc` is connected to it. Without it, `nc` will sit idle forever.
...
If you have gotten thus far, make sure to check out our [community forums](https://community.khulnasoft.com) to share your experience using Khulnasoft with StatsD.

## StatsD Step By Step Guide

In this guide, we'll go through a scenario of visualizing our data in Khulnasoft in a matter of seconds using 
...
the StatsD protocol. Simply enable the StatsD output and point it to the node that runs Khulnasoft, which is `localhost` in this case.

In general, the process for creating a StatsD collector can be summarized in 2 steps:

- Run an experiment by sending StatsD metrics to Khulnasoft, without any prior configuration. This will create 
  a chart per metric (called private charts) and will help you verify that everything works as expected from the application side of things.
    ...
    - Make sure to reload the dashboard tab **after** you start sending data to Khulnasoft.

- Create a configuration file for your app using [edit-config](/docs/khulnasoft-agent/configuration): `sudo ./edit-config
  statsd.d/myapp.conf`
...
- A node with the [Khulnasoft](/docs/khulnasoft-agent/installation) installed.
...
will be grouped in the same charts and how the charts will be grouped on Khulnasoft's dashboard.
...
In Khulnasoft, every metric will receive its own chart, called a `private chart`. Although in the 
...
[supported metrics](#metrics-supported-by-khulnasoft). Histograms and timers support mathematical operations 
...
Use [`edit-config`](/docs/khulnasoft-agent/configuration#edit-a-configuration-file-using-edit-config) 
...
Next, you need is to understand how to organize metrics in Khulnasoft’s StatsD.

#### Synthetic charts

Khulnasoft lets you group the metrics exposed by your instrumented application with _synthetic charts_.
...
Continue this dictionary process with any other metrics you want to collect with Khulnasoft.
...
Find more details about family and context in our [documentation](https://github.com/khulnasoft/khulnasoft/blob/master/src/web/README.md#families).
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns). 
  ...
  patterns in [dimension patterns](/docs/collecting-metrics/statsd#dimension-patterns).
...
If you define a chart, run Khulnasoft to visualize metrics, and then add or remove a dimension from that chart, 
this will result in a new chart with the same name, confusing Khulnasoft. If you change the dimensions of the chart, 
make sure to also change the `name` of that chart, since it serves as the `id` of that chart in Khulnasoft's storage. 
...
dashboard in Khulnasoft. We can do it in a few simple steps:
...
Note that Khulnasoft will report the rate for metrics and counters, even if k6 or another application 
sends an _absolute_ number. For example, k6 sends absolute HTTP requests with `http_reqs`, 
but Khulnasoft visualizes that in `requests/second`.

To enable this StatsD configuration, [restart Khulnasoft](/docs/khulnasoft-agent/maintenance/service-control).

### Final touches

At this point, you have used StatsD to gather metrics for k6, creating a whole new section in your 
Khulnasoft dashboard in the process. Moreover, you can further customize the icon of the particular section, 
    ...
    'system.cpu': {
        info: function (os) {
            ...
            return 'Total CPU utilization (all cores). 100% here means there is no CPU idle time at all. You can get per core usage at the <a href="#menu_cpu">CPUs</a> section and per application usage at the <a href="#menu_apps">Applications Monitoring</a> section.'
                + khulnasoftDashboard.sparkline('<br/>Keep an eye on <b>iowait</b> ', 'system.cpu', 'iowait', '%', '. If it is constantly high, your disks are a bottleneck and they slow your system down.')
                + khulnasoftDashboard.sparkline('<br/>An important metric worth monitoring, is <b>softirq</b> ', 'system.cpu', 'softirq', '%', '. A constantly high percentage of softirq may indicate network driver issues.');
        },
        valueRange: "[0, 100]"
    },
```
...
These descriptions can greatly help the Khulnasoft user who is monitoring your application in the midst of an incident. 

The `info` field supports `html`, embedding useful links and instructions in the description.

### Vendoring a new collector

While we docsed how to visualize any data source in Khulnasoft using the StatsD protocol, we have also created a new collector.

As long as you use the same underlying collector, every new `myapp.conf` file will create a new data 
source and dashboard section for Khulnasoft. Khulnasoft loads all the configuration files by default, but it will 
**not** create dashboard sections or charts, unless it starts receiving data for that particular data source. 
This means that we can now share our collector with the rest of the Khulnasoft community. 

- Make sure you follow the [contributing guide](/docs/developer-and-contributor-corner/contributing) 
- Fork the khulnasoft/khulnasoft repository
- Place the configuration file inside `khulnasoft/collectors/statsd.plugin`
- Add a reference in `khulnasoft/collectors/statsd.plugin/Makefile.am`. For example, if we contribute the `k6.conf` file:
```Makefile
