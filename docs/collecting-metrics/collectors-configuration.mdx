---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/collectors/REFERENCE.md"
sidebar_label: "Collectors configuration"
docs_status: "Published"
toc_max_heading_level: "4"
docs_rel_path: "Collecting Metrics"
sidebar_position: "700010"
docs_link: "https://docs.khulnasoft.com/docs/collecting-metrics/collectors-configuration"
---

# Collector configuration

Find available collectors in the [Collecting Metrics](/docs/collecting-metrics) guide and on our [Integrations page](https://www.khulnasoft.com/integrations).
...
> **Info**
>
> Enable and configure Go collectors directly through the UI using the [Dynamic Configuration Manager](/docs/khulnasoft-agent/configuration/dynamic-configuration-manager).
...
1. Open `khulnasoft.conf` using [`edit-config`](/docs/khulnasoft-agent/configuration#edit-a-configuration-file-using-edit-config).
...
2. Uncomment the collector's line and set it to `no`:
   ...
   modules:
       xyz_collector: no
   ```
3. [Restart](/docs/khulnasoft-agent/maintenance/service-control) the Agent after making changes.
...
1. Navigate to the plugins directory. If not found, check the `plugins directory` setting in `khulnasoft.conf`.
   ```bash
   cd /usr/libexec/khulnasoft/plugins.d/
   ```
2. Switch to the khulnasoft user.
   ```bash
