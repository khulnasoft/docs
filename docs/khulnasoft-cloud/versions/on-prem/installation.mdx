---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/khulnasoft-cloud/khulnasoft-cloud-on-prem/installation.md"
sidebar_label: "Installation"
docs_status: "Published"
docs_rel_path: "Khulnasoft Cloud/Versions/On-Prem"
sidebar_position: "504510"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-cloud/versions/on-prem/installation"
---

# Khulnasoft Cloud On-Prem Installation
...
| **Khulnasoft Cloud Services** | - 4 CPU cores<br/>- 15GiB memory<br/>- Note: Cloud services are ephemeral                                                                      |
...
All components below are included in the `khulnasoft-cloud-dependency` package:
...
3. **Pull Required Helm Charts**
   ...
   ```bash
   helm pull oci://362923047827.dkr.ecr.us-east-1.amazonaws.com/khulnasoft-cloud-dependency --untar  # Optional
   helm pull oci://362923047827.dkr.ecr.us-east-1.amazonaws.com/khulnasoft-cloud-onprem --untar
   ```
...
4. **Install Dependencies**
   ...
   The `khulnasoft-cloud-dependency` chart installs all required third-party applications. While we provide this for easy setup, **production environments should use their own configured versions of these components**:

    - Configure the installation by editing `values.yaml` in your `khulnasoft-cloud-dependency` chart directory.
    - Install the dependencies:
      ```bash
      cd [your helm chart location]
      helm upgrade --wait --install khulnasoft-cloud-dependency -n khulnasoft-cloud --create-namespace -f values.yaml .
      ```

5. **Install Khulnasoft Cloud On-Prem**

    - Configure the installation by editing `values.yaml` in your `khulnasoft-cloud-onprem` chart directory.
    - Install the application:
      ```bash
      cd [your helm chart location]
      helm upgrade --wait --install khulnasoft-cloud-onprem -n khulnasoft-cloud --create-namespace -f values.yaml .
      ```
   ...
   >
   > During the first installation, a `khulnasoft-cloud-common` secret is created containing critical encryption data. This secret persists through reinstalls and should never be deleted, as this will result in data loss.
...
| cloud-khulnasoft-assistant                | Exposes API endpoints to fetch a human-friendly explanation of various Khulnasoft configuration options, namely the Alerts.                                                                                                                                                                                                                                                                           |
