---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/observability-centralization-points/metrics-centralization-points/faq.md"
...
sidebar_position: "602060"
docs_link: "https://docs.khulnasoft.com/docs/observability-centralization-points/metrics-centralization-points/faq-on-metrics-cantralization-points"
---

# FAQ on Metrics Centralization Points

## How much can a Khulnasoft Parent node scale?

Khulnasoft Parents generally scale well. According [to our tests](https://blog.khulnasoft.com/khulnasoft-vs-prometheus-performance-analysis/) Khulnasoft Parents scale better than Prometheus for the same workload: -35% CPU utilization, -49% Memory Consumption, -12% Network Bandwidth, -98% Disk I/O, -75% Disk footprint.

For more information, Check [Sizing Khulnasoft Parents](/docs/observability-centralization-points/metrics-centralization-points/sizing-khulnasoft-parents).
...
No. The streaming protocol works on the same port as the internal web server of Khulnasoft Agents, but the protocol is not HTTP-friendly and cannot be understood by HTTP proxy servers.
...
We recommend using Khulnasoft Cloud to avoid receiving duplicate alert notifications. Khulnasoft Cloud deduplicates alert notifications so that you will receive them only once.

## When I have only Parents connected to Khulnasoft Cloud, will I be able to use the Functions feature on my child nodes?

Yes. Function requests will be received by the Parents and forwarded to the Child via their streaming connection. Function requests are propagated between parents, so this will work even if multiple levels of Khulnasoft Parents are involved.
...
Check [Restoring a Khulnasoft Parent after maintenance](/docs/observability-centralization-points/metrics-centralization-points/clustering-and-high-availability-of-khulnasoft-parents).

## I have a cluster of parents. Which one is used by Khulnasoft Cloud?

When there are multiple data sources for the same node, Khulnasoft Cloud follows this strategy:

1. Khulnasoft Cloud prefers Khulnasoft Agents having `live` data.
2. For time-series queries, when multiple Khulnasoft Agents have the retention required to answer the query, Khulnasoft Cloud prefers the one that is further away from production systems.
3. For Functions, Khulnasoft Cloud prefers Khulnasoft Agents that are closer to the production systems.
...
It depends on the ephemerality setting of each Khulnasoft Child.
...
To set the ephemeral flag on a node, edit its khulnasoft.conf and in the `[global]` section set `is ephemeral node = yes`. This setting is propagated to parent nodes and Khulnasoft Cloud.
...
A node can be forced into this "forgotten" state with the Khulnasoft CLI tool on the parent the node is connected to (if still connected) or one of the parent Agents it was previously connected to. The state will be propagated _upwards_ and _sideways_ in case of an HA setup.

```
khulnasoftcli remove-stale-node <node_id | machine_guid | hostname | ALL_NODES>
```

When using Khulnasoft Cloud (via a parent or directly), and a permanent node gets disconnected, Khulnasoft Cloud sends node disconnection notifications.
