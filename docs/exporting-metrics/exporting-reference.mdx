---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/exporting/README.md"
sidebar_label: "Exporting reference"
docs_status: "Published"
docs_rel_path: "Exporting Metrics"
description: "With the exporting engine, you can archive your Khulnasoft metrics to multiple external databases for long-term storage or further analysis."
sidebar_position: "800010"
docs_link: "https://docs.khulnasoft.com/docs/exporting-metrics/exporting-reference"
---

# Exporting reference

Welcome to the exporting engine reference guide. This guide contains comprehensive information about enabling,
configuring, and monitoring Khulnasoft's exporting engine, which allows you to send metrics to external time-series
databases.

For a quick introduction to the exporting engine's features, read our doc on [exporting metrics to time-series
databases](/docs/exporting-metrics/exporting-quickstart), or jump in to [enabling a connector](/docs/exporting-metrics/enable-an-exporting-connector).
...
When you enable the exporting engine and a connector, the Khulnasoft Agent exports metrics _beginning from the time you
restart its process_, not the entire [database of long-term metrics](/docs/khulnasoft-agent/database).

Since Khulnasoft collects thousands of metrics per server per second, which would easily congest any database server when
several Khulnasoft servers are sending data to it, Khulnasoft allows sending metrics at a lower frequency, by resampling them.

So, although Khulnasoft collects metrics every second, it can send to the external database servers averages or sums every
...
The exporting engine uses a number of connectors to send Khulnasoft metrics to external time-series databases. See our
[list of supported databases](/docs/exporting-metrics/exporting-quickstart#supported-databases) for information on which
connector to enable and configure for your database of choice.

- [**AWS Kinesis Data Streams**](/docs/exporting-metrics/aws-kinesis): Metrics are sent to the service in `JSON`
    format.
- [**Google Cloud Pub/Sub Service**](/docs/exporting-metrics/google-cloud-pub-sub): Metrics are sent to the service in `JSON`
    format.
- [**Graphite**](/docs/exporting-metrics/graphite): A plaintext interface. Metrics are sent to the database server as
    `prefix.hostname.chart.dimension`. `prefix` is configured below, `hostname` is the hostname of the machine (can
    also be configured). Docs more in our guide to [export and visualize Khulnasoft metrics in
    Graphite](/docs/exporting-metrics/graphite).
- [**JSON** document databases](/docs/exporting-metrics/json)
- [**OpenTSDB**](/docs/exporting-metrics/opentsdb): Use a plaintext or HTTP interfaces. Metrics are sent to
    OpenTSDB as `prefix.chart.dimension` with tag `host=hostname`.
- [**MongoDB**](/docs/exporting-metrics/mongodb): Metrics are sent to the database in `JSON` format.
- [**Prometheus**](/docs/exporting-metrics/prometheus): Use an existing Prometheus installation to scrape metrics
    from node using the Khulnasoft API.
- [**Prometheus remote write**](/docs/exporting-metrics/prometheus-remote-write). A binary snappy-compressed protocol
    buffer encoding over HTTP. Supports many [storage
    providers](https://prometheus.io/docs/operating/integrations/#remote-endpoints-and-storage).
- [**TimescaleDB**](https://github.com/khulnasoft/khulnasoft/blob/master/src/exporting/TIMESCALE.md): Use a community-built connector that takes JSON streams from a
    Khulnasoft client and writes them to a TimescaleDB table.

### Chart filtering

Khulnasoft can filter metrics, to send only a subset of the collected metrics. You can use the
configuration file
...
Khulnasoft supports three modes of operation for all exporting connectors:
...
- `average` sends to external databases normalized metrics from the Khulnasoft database. In this mode, all metrics
    are sent as gauges, in the units Khulnasoft uses. This abstracts data collection and simplifies visualization, but
    you will not be able to copy and paste queries from other sources to convert units. For example, CPU utilization
    percentage is calculated by Khulnasoft, so Khulnasoft will convert ticks to percentage and send the average percentage
    to the external database.

- `sum` or `volume`: the sum of the interpolated values shown on the Khulnasoft graphs is sent to the external
    database. So, if Khulnasoft is configured to send data to the database every 10 seconds, the sum of the 10 values
    shown on the Khulnasoft charts will be used.

Time-series databases suggest to collect the raw values (`as-collected`). If you plan to invest on building your
monitoring around a time-series database and you already know (or you will invest in docsing) how to convert units
and normalize the metrics in Grafana or other visualization tools, we suggest to use `as-collected`.

If, on the other hand, you just need long term archiving of Khulnasoft metrics and you plan to mainly work with
Khulnasoft, we suggest to use `average`. It decouples visualization from data collection, so it will generally be a lot
simpler. Furthermore, if you use `average`, the charts shown in the external service will match exactly what you
see in Khulnasoft, which is not necessarily true for the other modes of operation.

### Independent operation

This code is smart enough, not to slow down Khulnasoft, independently of the speed of the external database server.
...
[pubsub:my_pubsub_instance]
    ...
    credentials file = /etc/khulnasoft/pubsub_credentials.json
...
- `destination = host1 host2 host3 ...`, accepts **a space separated list** of hostnames, IPs (IPv4 and IPv6) and
     ports to connect to. Khulnasoft will use the **first available** to send the metrics.
...
```
   ...
   When multiple servers are defined, Khulnasoft will try the next one when the previous one fails.

   Khulnasoft also ships `nc-exporting.sh`, a script that can be used as a fallback exporting connector to save the
   metrics to disk and push them to the time-series database when it becomes available again. It can also be used to
   monitor / trace / debug the metrics Khulnasoft generates.
...
- `prefix = Khulnasoft`, is the prefix to add to all metrics.

- `update every = 10`, is the number of seconds between sending data to the external database. Khulnasoft will add some
    randomness to this number, to prevent stressing the external server when many Khulnasoft servers send data to the same
...
- `buffer on failures = 10`, is the number of iterations (each iteration is `update every` seconds) to buffer data,
    ...
    failures, data loss on the connector instance is expected (Khulnasoft will also log it).
...
- `send hosts matching = localhost *` includes one or more space separated patterns, using `*` as wildcard (any number
    ...
    `localhost`), allowing us to filter which hosts will be sent to the external database when this Khulnasoft is a central
    Khulnasoft aggregating multiple hosts. A pattern starting with `!` gives a negative match. So to match all hosts named
...
- `send names instead of ids = yes | no` controls the metric names Khulnasoft should send to the external database.
    Khulnasoft supports names and IDs for charts and dimensions. Usually IDs are unique identifiers as read by the system
...
- `send configured labels = yes | no` controls if host labels defined in the `[host labels]` section in `khulnasoft.conf`
...
Khulnasoft can send metrics to external databases using the TLS/SSL protocol. Unfortunately, some of
them does not support encrypted connections, so you will have to configure a reverse proxy to enable
HTTPS communication between Khulnasoft and an external database. You can set up a reverse proxy with
...
Khulnasoft creates five charts in the dashboard, under the **Khulnasoft Monitoring** section, to help you monitor the health
and performance of the exporting engine itself:

1. **Buffered metrics**, the number of metrics Khulnasoft added to the buffer for dispatching them to the
    external database server.

2. **Exporting data size**, the amount of data (in KB) Khulnasoft added the buffer.

3. **Exporting operations**, the number of operations performed by Khulnasoft.

4. **Exporting thread CPU usage**, the CPU resources consumed by the Khulnasoft thread, that is responsible for sending
