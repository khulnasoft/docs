---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/packaging/installer/methods/ansible.md"
sidebar_label: "Ansible"
docs_status: "Published"
docs_rel_path: "Khulnasoft Agent/Installation"
description: "Deploy an infrastructure monitoring solution in minutes with the Khulnasoft Agent and Ansible. Use and customize a simple playbook for monitoring as code."
sidebar_position: "403080"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/installation/ansible"
---

# Deploy Khulnasoft with Ansible

Khulnasoft's [one-line kickstart](/docs/khulnasoft-agent/installation) is zero-configuration, highly adaptable, and compatible with tons
...
deploy Khulnasoft across an entire infrastructure with many nodes? What if you want to make this deployment reliable,
...
operate. When you deploy Khulnasoft with Ansible, you're also deploying _monitoring as code_.

In this guide, we'll walk through the process of using an [Ansible
playbook](https://github.com/khulnasoft/community/tree/main/configuration-management/ansible-quickstart/) to automatically
deploy the Khulnasoft Agent to any number of distributed nodes, manage the configuration of each node, and connect them to
your Khulnasoft Cloud account. You'll go from some unmonitored nodes to a infrastructure monitoring solution in a matter of
minutes.

## Prerequisites

- A Khulnasoft Cloud account. [Sign in and create one](https://app.khulnasoft.com) if you don't have one already.
...
First, download the
[playbook](https://github.com/khulnasoft/community/tree/main/configuration-management/ansible-quickstart/), move it to the
current directory, and remove the rest of the cloned repository, as it's not required for using the Ansible playbook.

```bash
git clone https://github.com/khulnasoft/community.git
mv community/configuration-management/ansible-quickstart .
rm -rf community
```

Or if you don't want to clone the entire repository, use the [gitzip browser extension](https://gitzip.org/) to get the khulnasoft-agent-deployment directory as a zip file.
...
You can also set the `hostname` variable, which appears both on the local Agent dashboard and Khulnasoft Cloud, or you can
...
In order to connect your node(s) to your Space in Khulnasoft Cloud, and see all their metrics in real-time in composite
charts or perform [Metric
Correlations](/docs/machine-docsing-and-anomaly-detection/metric-correlations), you need to set the `claim_token`
and `claim_room` variables.

To find your `claim_token` and `claim_room`, go to Khulnasoft Cloud, then click on your Space's name in the top navigation,
...
Because we're connecting this node to Khulnasoft Cloud, and will view its dashboards there instead of via the IP address or
...
doc](/docs/security-and-privacy-design).

> Curious about why Khulnasoft's dashboard is open by default? Read our [blog
> post](https://www.khulnasoft.com/blog/khulnasoft-agent-dashboard/) on that zero-configuration design decision.
...
The task to install Khulnasoft will take a few minutes per node, so be patient! Once the playbook reaches the connect to Cloud
task, your nodes start populating your Space in Khulnasoft Cloud.
