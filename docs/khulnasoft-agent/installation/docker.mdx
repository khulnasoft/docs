---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/packaging/docker/README.md"
sidebar_label: "Docker"
docs_status: "Published"
docs_rel_path: "Khulnasoft Agent/Installation"
sidebar_position: "403010"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/installation/docker"
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Install Khulnasoft with Docker
...
## Create a new Khulnasoft Agent container
...
The Khulnasoft container requires different privileges and mounts to provide functionality similar to that provided by
Khulnasoft installed on the host. Below you can find a list of Khulnasoft components that need these privileges and mounts,
...
|:----------------------:|:--------------------------:|--------------------------------------------------------------------------------------------------------------------------------------------------|
|        khulnasoft         |      /etc/os-release       | Host info detection.                                                                                                                             |
...
|      go.d.plugin       |    /var/run/docker.sock    | Docker Engine and containers monitoring. See [docker](https://github.com/khulnasoft/go.d.plugin/tree/master/modules/docker#readme) collector.       |
|      go.d.plugin       |          /var/log          | Web servers logs tailing. See [weblog](https://github.com/khulnasoft/go.d.plugin/tree/master/modules/weblog#readme) collector.                      |
...
|      go.d.plugin       |         /run/dbus          | [go.d/systemdunits](https://github.com/khulnasoft/go.d.plugin/tree/master/modules/systemdunits#readme)                                              |

</details>

### Recommended way

Both methods create a [volume](https://docs.docker.com/storage/volumes/) for Khulnasoft's configuration files
_within the container_ at `/etc/khulnasoft`.
...
directory, start Khulnasoft by running `docker-compose up -d`.
...
> :bookmark_tabs: Note
>
> If you plan to connect the node to Khulnasoft Cloud, you can find the command with the right parameters by clicking the "Add Nodes" button in your Space's "Nodes" view.
...
- Allowing the Khulnasoft container to access GPU resources.
...
Add the following to the khulnasoft service.
...
Use a [bind mount](https://docs.docker.com/storage/bind-mounts/) for `/etc/khulnasoft` rather than a volume.

This example assumes that you’ve created `khulnasoftconfig/` in your home directory.

```bash
mkdir khulnasoftconfig
```
...
docker run -d --name=khulnasoft \
  --pid=host \
  --network=host \
  -v $(pwd)/khulnasoftconfig/khulnasoft:/etc/khulnasoft \
  -v khulnasoftlib:/var/lib/khulnasoft \
  -v khulnasoftcache:/var/cache/khulnasoft \
  -v /:/host/root:ro,rslave \
  -v /etc/passwd:/host/etc/passwd:ro \
...
services:
  khulnasoft:
    ...
    security_opt:
      - apparmor:unconfined
    volumes:
      - ./khulnasoftconfig/khulnasoft:/etc/khulnasoft
      - khulnasoftlib:/var/lib/khulnasoft
      - khulnasoftcache:/var/cache/khulnasoft
      - /:/host/root:ro,rslave
      - /etc/passwd:/host/etc/passwd:ro
...
should [secure the Khulnasoft instance](/docs/khulnasoft-agent/configuration/securing-agents). This
section contains an example of how to install Khulnasoft with an SSL reverse proxy and basic authentication.

You can use the following `docker-compose.yml` and Caddyfile files to run Khulnasoft with Docker. Replace the domains and
...
After setting Caddyfile run this with `docker-compose up -d` to have a fully functioning Khulnasoft setup behind an HTTP reverse
proxy.

Make sure Khulnasoft bind to docker0 interface if you've custom `web.bind to` setting in `khulnasoft.conf`.
...
services:
  caddy:
    image: caddy:2
    extra_hosts:
      - "host.docker.internal:host-gateway" # To access khulnasoft running with "network_mode: host".
    ports:
      - "80:80"
      - "443:443"
    volumes:
...
> **Note**: Using Khulnasoft with a Docker socket proxy might have some features not working as expected. It hasn't been fully tested by the Khulnasoft team.

Deploy a Docker socket proxy that accepts and filters out requests using something like
[HAProxy](/docs/khulnasoft-agent/configuration/securing-agents/running-the-agent-behind-a-reverse-proxy/haproxy) or
...
The reason it's safer to expose the socket to the proxy is because Khulnasoft has a TCP port exposed outside the Docker
...
Khulnasoft can be run successfully in a non-root environment, such as [rootless Docker](https://docs.docker.com/engine/security/rootless/).

However, it should be noted that Khulnasoft's data collection capabilities are considerably restricted in rootless Docker
due to its inherent limitations. While Khulnasoft can function in a rootless environment, it can’t access certain
...
This method creates a [volume](https://docs.docker.com/storage/volumes/) for Khulnasoft's configuration files
_within the container_ at `/etc/khulnasoft`.
See the [configure section](#configure-agent-containers) for details. If you want to access the configuration files from
your _host_ machine, see [host-editable configuration](#with-host-editable-configuration).
...
See our full list of Docker images at [Docker Hub](https://hub.docker.com/r/khulnasoft/khulnasoft).

The official `khulnasoft/khulnasoft` Docker image provides the following named tags:
...
If you started an Agent container using one of the [recommended methods](#create-a-new-khulnasoft-agent-container), and you
want to edit Khulnasoft's configuration, you must first use `docker exec` to attach to the container. Replace `khulnasoft`
with the name of your container.

```bash
docker exec -it khulnasoft bash
cd /etc/khulnasoft
./edit-config khulnasoft.conf
```

You need to restart the Agent to apply changes. Exit the container if you haven't already, then use the `docker` command
to restart the container: `docker restart khulnasoft`.

### Change the default hostname

You can change the hostname of a Docker container, and thus the name that appears in the local dashboard and in Khulnasoft
Cloud, when creating a new container. If you want to change the hostname of a Khulnasoft container _after_ you started it,
...
services:
  khulnasoft:
    image: khulnasoft/khulnasoft
    container_name: khulnasoft
    hostname: my_docker_compose_khulnasoft
```

If you don't want to destroy and recreate your container, you can edit the Agent's `khulnasoft.conf` file directly. See the
...
By default, the official Khulnasoft container images don’t include a number of optional runtime dependencies. You
can add these dependencies, or any other APT packages, at runtime by listing them in the environment variable
`KHULNASOFT_EXTRA_DEB_PACKAGES`.
...
You can control how the health checks run by using the environment variable `KHULNASOFT_HEALTHCHECK_TARGET` as follows:

- If left unset, the health check will attempt to access the `/api/v1/info` endpoint of the Agent.
- If set to the exact value 'cli', the health check script will use `khulnasoftcli ping` to determine if the Agent is
  running correctly or not. This is sufficient to ensure that Khulnasoft didn’t hang during startup, but doesn’t provide
...
At Khulnasoft, we provide multiple ways of testing your Docker images using your own repositories.
