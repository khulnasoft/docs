---
custom_edit_url: "https://github.com/khulnasoft/helmchart/edit/master/charts/khulnasoft/README.md"
sidebar_label: "Kubernetes Helm chart reference"
docs_status: "Published"
docs_rel_path: "Khulnasoft Agent/Installation"
sidebar_position: "403030"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/installation/kubernetes-helm-chart-reference"
---

# Khulnasoft Helm chart for Kubernetes deployments

<a href="https://artifacthub.io/packages/search?repo=khulnasoft" target="_blank" rel="noopener noreferrer"><img loading="lazy" src="https://img.shields.io/endpoint?url=https://artifacthub.io/badge/repository/khulnasoft" alt="Artifact HUB" class="img_node_modules-@docusaurus-theme-classic-lib-theme-MDXComponents-Img-styles-module"></img></a>

<img src="https://img.shields.io/badge/Version-3.7.124-informational" alt="Version: 3.7.124"></img>

<img loading="lazy" src="https://img.shields.io/badge/AppVersion-v2.2.5-informational" alt="AppVersion: v2.2.5" class="img_node_modules-@docusaurus-theme-classic-lib-theme-MDXComponents-Img-styles-module"></img>

_Based on the work of varyumin (https://github.com/varyumin/khulnasoft)_.

## Introduction

This chart bootstraps a [Khulnasoft](https://github.com/khulnasoft/khulnasoft) deployment on a [Kubernetes](http://kubernetes.io)
cluster using the [Helm](https://helm.sh) package manager.

By default, the chart installs:
- A Khulnasoft child pod on each node of a cluster, using a `Daemonset`
- A Khulnasoft k8s state monitoring pod on one node, using a `Deployment`. This virtual node is called `khulnasoft-k8s-state`.
- A Khulnasoft parent pod on one node, using a `Deployment`. This virtual node is called `khulnasoft-parent`.

Disabled by default:
- A Khulnasoft restarter `CronJob`. Its main purpose is to automatically update Khulnasoft when using nightly releases.
...
notifications, and provide the Khulnasoft UI to view metrics using an ingress controller.
...
To use Khulnasoft's Helm repository, run the following commands:
...
**See our [install Khulnasoft on Kubernetes](/docs/khulnasoft-agent/installation/kubernetes)
...
```console
git clone https://github.com/khulnasoft/helmchart.git khulnasoft-helmchart
```

To install the chart with the release name `khulnasoft`:

```console
helm install khulnasoft ./khulnasoft-helmchart/charts/khulnasoft
```
...
The following table lists the configurable parameters of the khulnasoft chart and their default values.

| Parameter                                     | Description                                                                                                                                            | Default                                                                                 |
|-----------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------|
| `kubeVersion`                                 | Kubernetes version                                                                                                                                     | Autodetected                                                                            |
| `replicaCount`                                | Number of `replicas` for the parent khulnasoft `Deployment`                                                                                               | `1`                                                                                     |
| `imagePullSecrets`                            | An optional list of references to secrets in the same namespace to use for pulling any of the images                                                   | `[]`                                                                                    |
| `image.repository`                            | Container image repo                                                                                                                                   | `khulnasoft/khulnasoft`                                                                       |
| `image.tag`                                   | Container image tag                                                                                                                                    | Latest stable khulnasoft release                                                           |
...
| `ingress.enabled`                             | Create Ingress to access the khulnasoft web UI                                                                                                            | `true`                                                                                  |
...
| `ingress.path`                                | URL path for the ingress. If changed, a proxy server needs to be configured in front of khulnasoft to translate path from a custom one to a `/`           | `/`                                                                                     |
...
| `ingress.hosts`                               | URL hostnames for the ingress (they need to resolve to the external IP of the ingress controller)                                                      | `khulnasoft.k8s.local`                                                                     |
...
| `serviceAccount.name`                         | The name of the service account to use. If not set and create is true, a name is generated using the fullname template.                                | `khulnasoft`                                                                               |
| `clusterrole.name`                            | Name of the cluster role linked with the service account                                                                                               | `khulnasoft`                                                                               |
| `APIKEY`                                      | The key shared between the parent and the child khulnasoft for streaming                                                                                  | `11111111-2222-3333-4444-555555555555`                                                  |
| `restarter.enabled`                           | Install CronJob to update Khulnasoft Pods                                                                                                                 | `false`                                                                                 |
...
| `parent.hostname`                             | Parent node hostname                                                                                                                                   | `khulnasoft-parent`                                                                        |
...
| `parent.database.storageclass`                | The storage class for the persistent volume claim of the parent's database store, mounted to `/var/cache/khulnasoft`                                      | the default storage class                                                               |
...
| `parent.alarms.storageclass`                  | The storage class for the persistent volume claim of the parent's alarm log, mounted to `/var/lib/khulnasoft`                                             | the default storage class                                                               |
...
| `parent.claiming.enabled`                     | Enable parent claiming for khulnasoft cloud                                                                                                               | `false`                                                                                 |
...
| `k8sState.hostname`                           | K8s state node hostname                                                                                                                                | `khulnasoft-k8s-state`                                                                     |
...
| `k8sState.persistence.enabled`                | Whether should use a persistent volume for `/var/lib/khulnasoft`                                                                                          | `true`                                                                                  |
| `k8sState.persistence.storageclass`           | The storage class for the persistent volume claim of  `/var/lib/khulnasoft`                                                                               | the default storage class                                                               |
| `k8sState.persistence.volumesize`             | The storage space for the PVC of `/var/lib/khulnasoft`                                                                                                    | `1Gi`                                                                                   |
...
| `k8sState.claiming.enabled`                   | Enable claiming for khulnasoft cloud                                                                                                                      | `false`                                                                                 |
...
| `child.persistence.hostPath`                  | Host node directory for storing child instance data                                                                                                    | `/var/lib/khulnasoft-k8s-child`                                                            |
| `child.persistence.enabled`                   | Whether or not to persist `/var/lib/khulnasoft` in the `child.persistence.hostPath`.                                                                      | `true`                                                                                  |
...
| `child.claiming.enabled`                      | Enable child claiming for khulnasoft cloud                                                                                                                | `false`                                                                                 |
...
| `sd.image.repository`                         | Service-discovery image repo                                                                                                                           | `khulnasoft/agent-sd`                                                                      |
...
| `sd.child.enabled`                            | Add service-discovery sidecar container to the khulnasoft child pod definition                                                                            | `true`                                                                                  |
...
| `sd.child.configmap.name`                     | Child service-discovery ConfigMap name                                                                                                                 | `khulnasoft-child-sd-config-map`                                                           |
...
```console
$ helm install ./khulnasoft --name my-release \
...
```console
$ helm install ./khulnasoft --name my-release -f values.yaml
```
...
| Parameter                         | Description                                                                           | Default                                                                                                                       |
|-----------------------------------|---------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|
| `parent.configs.khulnasoft`          | Contents of the parent's `khulnasoft.conf`                                               | `memory mode = dbengine`                                                                                                      |
...
| `k8sState.configs.khulnasoft`        | Contents of  `khulnasoft.conf`                                                           | No persistent storage, no alarms                                                                                              |
| `k8sState.configs.stream`         | Contents of `stream.conf`                                                             | Send metrics to the parent at khulnasoft:{{ service.port }}                                                                      |
...
| `child.configs.khulnasoft`           | Contents of the child's `khulnasoft.conf`                                                | No persistent storage, no alarms, no UI                                                                                       |
| `child.configs.stream`            | Contents of the child's `stream.conf`                                                 | Send metrics to the parent at khulnasoft:{{ service.port }}                                                                      |
...
To deploy additional khulnasoft user configuration files, you will need to add similar entries to either
the `parent.configs` or the `child.configs` arrays. Regardless of whether you add config files that reside directly
under `/etc/khulnasoft` or in a subdirectory such as `/etc/khulnasoft/go.d`, you can use the already provided configurations
...
the alarm and alarm template definitions, under `/etc/khulnasoft/health.d`.
...
2. alarms (`/var/lib/khulnasoft`) - alarm log, if not persistent pod recreation will result in parent appearing as a new
   node in `khulnasoft.com` (due to `./registry/` and `./cloud.d/` being removed).
...
Khulnasoft's [service discovery](https://github.com/khulnasoft/agent-service-discovery/), which is installed as part of the
...
- [ActiveMQ](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/activemq/README.md)
- [Apache](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/apache/README.md)
- [Bind](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/bind/README.md)
- [CockroachDB](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/cockroachdb/README.md)
- [Consul](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/consul/README.md)
- [CoreDNS](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/coredns/README.md)
- [Elasticsearch](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/elasticsearch/README.md)
- [Fluentd](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/fluentd/README.md)
- [FreeRADIUS](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/freeradius/README.md)
- [HDFS](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/hdfs/README.md)
- [Lighttpd](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/lighttpd/README.md)
- [Logstash](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/logstash/README.md)
- [MySQL](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/mysql/README.md)
- [NGINX](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/nginx/README.md)
- [OpenVPN](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/openvpn/README.md)
- [PHP-FPM](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/phpfpm/README.md)
- [RabbitMQ](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/rabbitmq/README.md)
- [Solr](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/solr/README.md)
- [Tengine](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/tengine/README.md)
- [Unbound](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/unbound/README.md)
- [VerneMQ](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/vernemq/README.md)
- [ZooKeeper](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/zookeeper/README.md)

#### Prometheus endpoints

Service discovery supports Prometheus endpoints via
the [Prometheus](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/collectors/go.d.plugin/modules/prometheus/README.md) collector.
...
the [default ConfigMap](https://github.com/khulnasoft/helmchart/blob/master/sdconfig/child.yml) that is shipped with the
Helmchart and deploy that to your cluster.

First, copy `khulnasoft-helmchart/sdconfig/child.yml` to a new location outside the `khulnasoft-helmchart` directory. The
destination can be anywhere you like, but the following examples assume it resides next to the `khulnasoft-helmchart`
directory.

```bash
cp khulnasoft-helmchart/sdconfig/child.yml .
```

Edit the new `child.yml` file according to your needs. See
the [Helm chart configuration](https://github.com/khulnasoft/helmchart#configuration) and the file itself for details. You
can then run
...
```bash
helm install --set-file sd.child.configmap.from.value=./child.yml khulnasoft ./khulnasoft-helmchart/charts/khulnasoft
helm upgrade --set-file sd.child.configmap.from.value=./child.yml khulnasoft ./khulnasoft-helmchart/charts/khulnasoft
```
...
For example, suppose you're installing Khulnasoft on all your database nodes, and you'd like the child pods to be labeled
...
$ helm install \
  ...
  khulnasoft ./khulnasoft-helmchart/charts/khulnasoft
```
...
- This repository is under the [Khulnasoft Code Of Conduct](/docs/developer-and-contributor-corner/community-code-of-conduct).
- Chat about your contribution and let us help you in
  our [forum](https://community.khulnasoft.com/c/agent-development/9)!
