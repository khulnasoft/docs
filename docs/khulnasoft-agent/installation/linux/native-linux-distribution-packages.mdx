---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/packaging/installer/methods/packages.md"
sidebar_label: "Native Linux Distribution packages"
docs_status: "Published"
docs_rel_path: "Khulnasoft Agent/Installation/Linux"
description: "Instructions for how to install Khulnasoft using native DEB or RPM packages."
sidebar_position: "402510"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/installation/linux/native-linux-distribution-packages"
---

# Install Khulnasoft using native DEB/RPM packages

For most common Linux distributions that use either DEB or RPM packages, Khulnasoft provides pre-built native packages
for current releases in-line with
our [official platform support policy](/docs/khulnasoft-agent/versions-&-platforms).
These packages will be used by default when attempting to install on a supported platform using our
[kickstart.sh installer script](/docs/khulnasoft-agent/installation/linux).
...
Khulnasoft’s official RPM repositories are hosted at <https://repository.khulnasoft.com/repos/index.html>. We provide four groups of
repositories at that top level:

- `stable`: Contains packages for stable releases of the Khulnasoft Agent.
- `edge`: Contains packages for nightly builds of the Khulnasoft Agent.
- `repoconfig`: Provides packages that set up configuration files for using the other repositories.
- `devel`: Is used for one-off development builds of the Khulnasoft Agent, and can simply be ignored by users.
...
<https://repository.khulnasoft.com/repos/stable/el/9/x86_64/>

Our RPM packages and repository metadata are signed using a GPG key with a user name of ‘Khulnasoftbot’. The
current key fingerprint is `6588FDD7B14721FE7C3115E6F9177B5265F56346`. The associated public key can be fetched from
`https://repository.khulnasoft.com/khulnasoftbot.gpg.key`.

If you are explicitly configuring a system to use our repositories, the recommended setup is to download the
appropriate repository configuration package from <https://repository.khulnasoft.com/repos/repoconfig/index.html>
...
> **Note**
>
> On RHEL and other systems that use the `el` repositories, some of the dependencies for Khulnasoft can only be found
...
> to manually install `epel-release` to be able to successfully install the Khulnasoft packages.

## Manual setup of DEB packages

Khulnasoft’s official DEB repositories are hosted at <https://repository.khulnasoft.com/repos/index.html>. We provide four groups of
repositories at that top level:
...
`http://repository.khulnasoft.com/repos/edge/ubuntu/focal/by-hash/SHA256/91ccff6523a3c4483ebb539ff2b4adcd3b6b5d0c0c2c9573c5a6947a127819bc`,
...
```text
deb by-hash=yes http://repository.khulnasoft.com/repos/stable/debian/ bullseye/
```
...
```text
Types: deb
URIs: http://repository.khulnasoft.com/repos/stable/debian/
Suites: bullseye/
By-Hash: Yes
Enabled: Yes
```
...
Our DEB packages and repository metadata are signed using a GPG key with a user name of ‘Khulnasoftbot’. The
current key fingerprint is `6588FDD7B14721FE7C3115E6F9177B5265F56346`. The associated public key can be fetched from
`https://repository.khulnasoft.com/khulnasoftbot.gpg.key`.

If you are explicitly configuring a system to use our repositories, the recommended setup is to download the
appropriate repository configuration package from <https://repository.khulnasoft.com/repos/repoconfig/index.html> and install it
...
## Local mirrors of the official Khulnasoft repositories
...
2. Using a regular website mirroring tool, such as GNU wget’s `--mirror` option. For this approach, simply point
   your mirroring tool at `https://repository.khulnasoft.com/repos/`, and everything should just work.
...
- If you intend to use our existing GPG signatures on the repository metadata and packages, you probably also want
  a local copy of our public GPG key, which can be fetched from `https://repository.khulnasoft.com/khulnasoftbot.gpg.key`.

## Public mirrors of the official Khulnasoft repositories
