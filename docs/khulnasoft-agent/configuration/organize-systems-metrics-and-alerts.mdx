---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/khulnasoft-agent/configuration/organize-systems-metrics-and-alerts.md"
sidebar_label: "Organize systems, metrics, and alerts"
docs_status: "Published"
docs_rel_path: "Khulnasoft Agent/Configuration"
sidebar_position: "409010"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/configuration/organize-systems-metrics-and-alerts"
---

# Organize systems, metrics, and alerts

When you use Khulnasoft to monitor and troubleshoot an entire infrastructure, you need sophisticated ways of keeping everything organized.
Khulnasoft allows organizing your observability infrastructure with Spaces, Rooms, virtual nodes, host labels, and metric labels.

## Spaces and Rooms

[Spaces](/docs/khulnasoft-cloud/spaces-and-rooms#spaces) are used for organization-level or infrastructure-level
grouping of nodes and people. A node can only appear in a single space, while people can have access to multiple spaces.

The [Rooms](/docs/khulnasoft-cloud/spaces-and-rooms#rooms) in a space bring together nodes and people in
collaboration areas. Rooms can also be used for fine-tuned
[role-based access control](/docs/khulnasoft-cloud/authentication-&-authorization/role-based-access-model).

## Virtual nodes

Khulnasoft’s virtual nodes functionality allows you to define nodes in configuration files and have them be treated as regular nodes
...
* Define virtual nodes in `/etc/khulnasoft/vnodes/vnodes.conf`
...
Let's take a peek into how to create host labels and apply them across a few of Khulnasoft's features to give you more
organization power over your infrastructure.

### Default labels

When Khulnasoft starts, it captures relevant information about the system and converts them into automatically generated
...
* Whether Khulnasoft is running inside a container, and if so, the OS and hardware details about the container's host
* Whether Khulnasoft is running inside K8s node
...
Host labels are defined in `khulnasoft.conf`. To create host labels, open that file using `edit-config`.
...
Once you've written a few host labels, you need to enable them. Instead of restarting the entire Khulnasoft service, you
can reload labels using the helpful `khulnasoftcli` tool:

```bash
khulnasoftcli reload-labels
```
...
By applying labels to exported metrics, you can more easily parse historical metrics with the labels applied. To docs
more about exporting, read the [documentation](/docs/exporting-metrics/exporting-reference).

## Metric labels

The Khulnasoft aggregate charts allow you to filter and group metrics based on label name-value pairs.
...
For example, suppose we have a single Khulnasoft Agent, collecting data from two remote Apache web servers, located in different data centers.
