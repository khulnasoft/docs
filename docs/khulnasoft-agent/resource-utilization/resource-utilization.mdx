---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/khulnasoft-agent/sizing-khulnasoft-agents/README.md"
...
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/resource-utilization/resource-utilization"
---

# Resource utilization

Khulnasoft is designed to automatically adjust its resource consumption based on the specific workload.

This table shows the specific system resources affected by different Khulnasoft features:
...
2. **Sample frequency**
    ...
    - **Impact**: Khulnasoft collects most metrics with 1-second granularity. This high frequency impacts CPU usage.
...
5. **Machine Docsing**
    ...
    - **Impact**: Machine docsing model training is CPU-intensive, affecting overall CPU usage.
    - **Optimization**: Consider disabling machine docsing for less critical metrics or adjusting model training frequency.
...
## Minimizing the resources used by Khulnasoft Agents
...
Check [RAM Requirements](/docs/khulnasoft-agent/resource-utilization/ram) for more information.

## Khulnasoft's performance and scalability optimization techniques

1. **Minimal Disk I/O**

   Khulnasoft directly writes metric data to disk, bypassing system caches and reducing I/O overhead. Additionally, its optimized data structures minimize disk space and memory usage through efficient compression and timestamping.

2. **Compact Storage Engine**

   Khulnasoft uses a custom 32-bit floating-point format tailored for efficient storage of time-series data, along with an anomaly bit. This, combined with a fixed-step database design, enables efficient storage and retrieval of data.
...
3. **Intelligent Query Engine**
   ...
   Khulnasoft prioritizes interactive queries over background tasks like machine docsing and replication, ensuring optimal user experience, especially under heavy load.

4. **Efficient Label Storage**

   Khulnasoft uses pointers to reference shared label key-value pairs, minimizing memory usage, especially in highly dynamic environments.

5. **Scalable Streaming Protocol**

   Khulnasoft's streaming protocol enables the creation of distributed monitoring setups, where Children offload data processing to Parents, optimizing resource utilization.
