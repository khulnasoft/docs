---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/khulnasoft-agent/versions-and-platforms.md"
sidebar_label: "Versions & Platforms"
docs_status: "Published"
docs_rel_path: "Khulnasoft Agent"
description: "Present all the supported platform in the Khulnasoft solution"
sidebar_position: "400010"
docs_link: "https://docs.khulnasoft.com/docs/khulnasoft-agent/versions-&-platforms"
---

# Khulnasoft Agent Versions & Platforms

Khulnasoft is evolving rapidly and new features are added at a constant pace. Therefore, we have a frequent release cadence to deliver all these features to use as soon as possible.

Khulnasoft Agents are available in 2 versions:
...
> IMPORTANT: Linux distributions frequently provide binary packages of Khulnasoft. However, the packages you will find in the distributions' repositories may be outdated, incomplete, missing significant features or completely broken. We recommend using the packages we provide.

## Third-party Supported Binary Packages

The following distributions always provide the latest stable version of Khulnasoft:
...
We guarantee Khulnasoft builds from source for the platforms we provide automated binary packages. These platforms are automatically checked via our CI, and fixes are always applied to allow merging new code into the nightly versions.
...
The static builds of Khulnasoft can be used on any Linux platform of the supported architectures. The only requirement these static builds have is a working Linux kernel, any version. Everything else required for Khulnasoft to run is inside the package itself.
...
When platforms are removed from the [Binary Distribution Packages](/docs/khulnasoft-agent/installation/linux/static-binary-linux-packages) list, they default to install or update Khulnasoft to a static build. This may mean that after platforms become EOL, Khulnasoft on them may lose some of its features. We recommend upgrading the operating system before it becomes EOL, to continue using all the features of Khulnasoft.
