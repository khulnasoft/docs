---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/ml/ml-configuration.md"
sidebar_label: "ML Configuration"
docs_status: "Published"
docs_rel_path: "Machine Docsing and Anomaly Detection"
sidebar_position: "1300020"
docs_link: "https://docs.khulnasoft.com/docs/machine-docsing-and-anomaly-detection/ml-configuration"
---

# ML Configuration

Khulnasoft's [Machine Docsing](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection) capabilities are enabled by default if the [Database mode](/docs/khulnasoft-agent/database) is set to `db = dbengine`

To enable or disable Machine Docsing capabilities on a node:

1. [Edit `khulnasoft.conf`](/docs/khulnasoft-agent/configuration#edit-a-configuration-file-using-edit-config)
2. In the `[ml]` section, set `enabled` to `yes` to enable ML, `no` to disable it, or leave it at the default `auto` to enable ML only when [Database mode](/docs/khulnasoft-agent/database) is set to `dbengine`
3. [Restart Khulnasoft](/docs/khulnasoft-agent/maintenance/service-control)
...
[ml]
        ...
        # charts to skip from training = khulnasoft.*
...
This example assumes 3 child nodes [streaming](/docs/observability-centralization-points/metrics-centralization-points) to 1 parent node and illustrates the main ways you might want to configure running ML for the children on the parent, running ML on the children themselves, or even a mix of approaches.

```mermaid
flowchart BT
    C1["Khulnasoft Child 0
    ML enabled"]
    C2["Khulnasoft Child 1
    ML enabled"]
    C3["Khulnasoft Child 2
    ML disabled"]
    P1["Khulnasoft Parent
...
# child 1 will run its own ML at the edge, even though parent will also run ML for it, a bit wasteful potentially to run ML in both places but is possible (Khulnasoft Cloud will essentially average any overlapping models).
...
- `enabled`: `yes` to enable, `no` to disable, or `auto` to let Khulnasoft decide when to enable ML.
...
- `charts to skip from training`: This parameter allows you to exclude certain charts from anomaly detection. By default, only khulnasoft related charts are excluded. This is to avoid the scenario where accessing the khulnasoft dashboard could itself trigger some anomalies if you don't access them regularly. If you want to include charts that are excluded by default, add them in small groups and then measure any impact on performance before adding additional ones. Example: If you want to include system, apps, and user charts:`!system.* !apps.* !user.* *`.
