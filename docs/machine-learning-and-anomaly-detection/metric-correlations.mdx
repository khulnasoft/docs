---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/metric-correlations.md"
...
sidebar_position: "1300030"
docs_link: "https://docs.khulnasoft.com/docs/machine-docsing-and-anomaly-detection/metric-correlations"
---
...
By displaying the standard Khulnasoft dashboard, filtered to show only charts that are relevant to the window of interest, you can get to the root cause sooner.
...
When viewing the [Metrics tab or a single-node dashboard](/docs/dashboards-and-charts/metrics-tab-and-single-node-tabs), the **Metric Correlations** button appears in the top right corner of the page.

To start correlating metrics, click the **Metric Correlations** button, [highlight a selection of metrics](/docs/dashboards-and-charts/khulnasoft-charts#highlight) on a single chart. The selected timeframe needs at least 15 seconds for Metric Correlation to work.
...
MC enables a few input parameters that users can define to iteratively explore their data in different ways. As is usually the case in Machine Docsing (ML), there is no "one size fits all" algorithm, what approach works best will typically depend on the type of data (which can be very different from one metric to the next) and even the nature of the event or incident you might be exploring in Khulnasoft.
...
- `KS2` - A statistical test ([Two-sample Kolmogorov Smirnov](https://en.wikipedia.org/wiki/Kolmogorov%E2%80%93Smirnov_test#Two-sample_Kolmogorov%E2%80%93Smirnov_test)) comparing the distribution of the highlighted window to the baseline to try and quantify which metrics have most evidence of a significant change. You can explore our implementation [here](https://github.com/khulnasoft/khulnasoft/blob/d917f9831c0a1638ef4a56580f321eb6c9a88037/database/metric_correlations.c#L212).
- `Volume` - A heuristic measure based on the percentage change in averages between a highlighted window and baseline, with various edge cases sensibly controlled for. You can explore our implementation [here](https://github.com/khulnasoft/khulnasoft/blob/d917f9831c0a1638ef4a56580f321eb6c9a88037/database/metric_correlations.c#L516).

### Aggregation

Behind the scenes, Khulnasoft will aggregate the raw data as needed such that arbitrary window lengths can be selected for MC. By default, Khulnasoft will just `Average` raw data when needed as part of pre-processing. However, other aggregations like `Median`, `Min`, `Max`, `Stddev` are also possible.

### Data

Unlike other observability agents that only collect raw metrics, Khulnasoft also assigns an [Anomaly Bit](https://github.com/khulnasoft/khulnasoft/tree/master/src/ml#anomaly-bit) in real-time. This bit flags whether a metric is within normal ranges (0) or deviates significantly (1). This built-in anomaly detection allows for the analysis of both the raw data and the anomaly rates.

**Note**: Read more [here](/docs/machine-docsing-and-anomaly-detection/ml-models-and-anomaly-detection) to docs more about the native anomaly detection features within khulnasoft.
...
Metric Correlations (MC) requests to Khulnasoft Cloud are handled in two ways:
...
- When running Metric Correlations from the [Metrics tab](/docs/dashboards-and-charts/metrics-tab-and-single-node-tabs) across multiple nodes, you might find better results if you iterate on the initial results by grouping by node to then filter to nodes of interest and rerun the Metric Correlations. So a typical workflow in this case would be to:
    ...
    - If you see a subset of nodes clearly jump out when you group by node, then filter for just those nodes of interest and run the MC again. This will result in less aggregation needing to be done by Khulnasoft and so should help give clearer results as you interact with the slider.
