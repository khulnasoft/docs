---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/plugins.d/README.md"
sidebar_label: "External plugins"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner/External-plugins"
sidebar_position: "1606000"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/external-plugins/external-plugins"
---

# External plugins

`plugins.d` is the Khulnasoft internal plugin that collects metrics
from external processes, thus allowing Khulnasoft to use **external plugins**.

## Provided External Plugins

|                                                 plugin                                                 | language |      O/S       | description                                                                                                                             |
|:------------------------------------------------------------------------------------------------------:|:--------:|:--------------:|:----------------------------------------------------------------------------------------------------------------------------------------|
|     [apps.plugin](https://github.com/khulnasoft/khulnasoft/blob/master/src/collectors/apps.plugin/README.md)     |   `C`    | linux, freebsd | monitors the whole process tree on Linux and FreeBSD and breaks down system resource usage by **process**, **user** and **user group**. |
| [charts.d.plugin](/docs/developer-and-contributor-corner/external-plugins/charts.d.plugin) |  `BASH`  |      all       | a **plugin orchestrator** for data collection modules written in `BASH` v4+.                                                            |
|     [cups.plugin](/docs/collecting-metrics/hardware-devices-and-sensors/cups)     |   `C`    |      all       | monitors **CUPS**                                                                                                                       |
|     [ebpf.plugin](https://github.com/khulnasoft/khulnasoft/blob/master/src/collectors/ebpf.plugin/README.md)     |   `C`    |     linux      | monitors different metrics on environments using kernel internal functions.                                                             |
|              [go.d.plugin](/docs/developer-and-contributor-corner/external-plugins/go.d.plugin)               |   `GO`   |      all       | collects metrics from the system, applications, or third-party APIs.                                                                    |
|   [ioping.plugin](/docs/collecting-metrics/synthetic-checks/ioping)   |   `C`    |      all       | measures disk latency.                                                                                                                  |
| [freeipmi.plugin](/docs/collecting-metrics/hardware-devices-and-sensors/intelligent-platform-management-interface-ipmi) |   `C`    |     linux      | collects metrics from enterprise hardware sensors, on Linux servers.                                                                    |
|   [nfacct.plugin](/docs/collecting-metrics/linux-systems/firewall/netfilter)   |   `C`    |     linux      | collects netfilter firewall, connection tracker and accounting metrics using `libmnl` and `libnetfilter_acct`.                          |
|  [xenstat.plugin](/docs/collecting-metrics/containers-and-vms/xen-xcp-ng)  |   `C`    |     linux      | collects XenServer and XCP-ng metrics using `lxenstat`.                                                                                 |
|     [perf.plugin](/docs/collecting-metrics/linux-systems/cpu-performance)     |   `C`    |     linux      | collects CPU performance metrics using performance monitoring units (PMU).                                                              |
| [python.d.plugin](/docs/developer-and-contributor-corner/external-plugins/python.d.plugin) | `python` |      all       | a **plugin orchestrator** for data collection modules written in `python` v2 or v3 (both are supported).                                |
| [slabinfo.plugin](/docs/collecting-metrics/linux-systems/kernel/linux-kernel-slab-allocator-statistics) |   `C`    |     linux      | collects kernel internal cache objects (SLAB) metrics.                                                                                  |

Plugin orchestrators may also be described as **modular plugins**. They are modular since they accept custom made modules to be included. Writing modules for these plugins is easier than accessing the native Khulnasoft API directly. You will find modules already available for each orchestrator under the directory of the particular modular plugin (e.g. under python.d.plugin for the python orchestrator).
...
This plugin allows Khulnasoft to use **external plugins** for data collection:
...
2.  external data collection plugins may use O/S capabilities or `setuid` to
    run with escalated privileges (compared to the `khulnasoft` daemon).
    The communication between the external plugin and Khulnasoft is unidirectional
    (from the plugin to Khulnasoft), so that Khulnasoft cannot manipulate an external
    plugin running with escalated privileges.

## Operation

Each of the external plugins is expected to run forever.
Khulnasoft will start it when it starts and stop it when it exits.

If the external plugin exits or crashes, Khulnasoft will log an error.
If the external plugin exits or crashes without pushing metrics to Khulnasoft, Khulnasoft will not start it again.
...
-   Plugins may also be disabled by Khulnasoft if they output things that Khulnasoft does not understand.

The `stdout` of external plugins is connected to Khulnasoft to receive metrics,
with the API defined below.

The `stderr` of external plugins is connected to Khulnasoft's `error.log`.
...
Khulnasoft will supply the environment variables `KHULNASOFT_USER_CONFIG_DIR` (for user supplied) and `KHULNASOFT_STOCK_CONFIG_DIR` (for Khulnasoft supplied) configuration files to identify the directory where configuration files are stored. It is up to the plugin to read the configuration it needs.

The `khulnasoft.conf` section `[plugins]` section contains a list of all the plugins found at the system where Khulnasoft runs, with a boolean setting to enable them or not.
...
`/usr/libexec/khulnasoft/plugins.d`. New plugins can be added any time, and Khulnasoft will detect them in a timely manner.

For each of the external plugins enabled, another `khulnasoft.conf` section
...
Khulnasoft will provide to the external plugins the environment variable `KHULNASOFT_UPDATE_EVERY`, in seconds (the default is 1). This is the **minimum update frequency** for all charts. A plugin that is updating values more frequently than this, is just wasting resources.

Khulnasoft will call the plugin with just one command line parameter: the number of seconds the user requested this plugin to update its data (by default is also 1).

Other than the above, the plugin configuration is up to the plugin.

Keep in mind, that the user may use Khulnasoft configuration to overwrite chart and dimension parameters. This is transparent to the plugin.

### Autoconfiguration

Plugins should attempt to autoconfigure themselves when possible.

For example, if your plugin wants to monitor `squid`, you can search for it on port `3128` or `8080`. If any succeeds, you can proceed. If it fails you can output an error (on stderr) saying that you cannot find `squid` running and giving instructions about the plugin configuration. Then you can stop (exit with non-zero value), so that Khulnasoft will not attempt to start the plugin again.

## External Plugins API

Any program that can print a few values to its standard output can become a Khulnasoft external plugin.

Khulnasoft parses lines starting with:
...
Khulnasoft may send the following commands to the plugin's `stdin`:
...
expected to update the values for its charts**. The value passed by Khulnasoft
...
There are a few environment variables that are set by `khulnasoft` and are
available for the plugin to use.

|             variable             | description                                                                                                                                                                                                                                            |
|:--------------------------------:|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    `KHULNASOFT_USER_CONFIG_DIR`     | The directory where all Khulnasoft-related user configuration should be stored. If the plugin requires custom user configuration, this is the place the user has saved it (normally under `/etc/khulnasoft`).                                                |
|    `KHULNASOFT_STOCK_CONFIG_DIR`    | The directory where all Khulnasoft -related stock configuration should be stored. If the plugin is shipped with configuration files, this is the place they can be found (normally under `/usr/lib/khulnasoft/conf.d`).                                      |
|      `KHULNASOFT_PLUGINS_DIR`       | The directory where all Khulnasoft plugins are stored.                                                                                                                                                                                                    |
|   `KHULNASOFT_USER_PLUGINS_DIRS`    | The list of directories where custom plugins are stored.                                                                                                                                                                                               |
|        `KHULNASOFT_WEB_DIR`         | The directory where the web files of Khulnasoft are saved.                                                                                                                                                                                                |
|       `KHULNASOFT_CACHE_DIR`        | The directory where the cache files of Khulnasoft are stored. Use this directory if the plugin requires a place to store data. A new directory should be created for the plugin for this purpose, inside this directory.                                  |
|        `KHULNASOFT_LOG_DIR`         | The directory where the log files are stored. By default the `stderr` output of the plugin will be saved in the `error.log` file of Khulnasoft.                                                                                                           |
|      `KHULNASOFT_HOST_PREFIX`       | This is used in environments where system directories like `/sys` and `/proc` have to be accessed at a different path.                                                                                                                                 |
|      `KHULNASOFT_DEBUG_FLAGS`       | This is a number (probably in hex starting with `0x`), that enables certain Khulnasoft debugging features. Check **\[[Tracing Options]]** for more information.                                                                                           |
|      `KHULNASOFT_UPDATE_EVERY`      | The minimum number of seconds between chart refreshes. This is like the **internal clock** of Khulnasoft (it is user configurable, defaulting to `1`). There is no meaning for a plugin to update its values more frequently than this number of seconds. |
|     `KHULNASOFT_INVOCATION_ID`      | A random UUID in compact form, representing the unique invocation identifier of Khulnasoft. When running under systemd, Khulnasoft uses the `INVOCATION_ID` set by systemd.                                                                                  |
|       `KHULNASOFT_LOG_METHOD`       | One of `syslog`, `journal`, `stderr` or `none`, indicating the preferred log method of external plugins.                                                                                                                                               |
|       `KHULNASOFT_LOG_FORMAT`       | One of `journal`, `logfmt` or `json`, indicating the format of the logs. Plugins can use the Khulnasoft `systemd-cat-native` command to log always in `journal` format, and have it automatically converted to the format expected by khulnasoft.            |
|       `KHULNASOFT_LOG_LEVEL`        | One of `emergency`, `alert`, `critical`, `error`, `warning`, `notice`, `info`, `debug`. Plugins are expected to log events with the given priority and the more important ones.                                                                        |
|    `KHULNASOFT_SYSLOG_FACILITY`     | Set only when the `KHULNASOFT_LOG_METHOD` is `syslog`. Possible values are `auth`, `authpriv`, `cron`, `daemon`, `ftp`, `kern`, `lpr`, `mail`, `news`, `syslog`, `user`, `uucp` and `local0` to `local7`                                                  |  
| `KHULNASOFT_ERRORS_THROTTLE_PERIOD` | The log throttling period in seconds.                                                                                                                                                                                                                  |
|   `KHULNASOFT_ERRORS_PER_PERIOD`    | The allowed number of log events per period.                                                                                                                                                                                                           | 
| `KHULNASOFT_SYSTEMD_JOURNAL_PATH`   | When `KHULNASOFT_LOG_METHOD` is set to `journal`, this is the systemd-journald socket path to use.                                                                                                                                                        |

### The output of the plugin

The plugin should output instructions for Khulnasoft to its output (`stdout`). Since this uses pipes, please make sure you flush stdout after every iteration.

#### DISABLE

`DISABLE` will disable this plugin. This will prevent Khulnasoft from restarting the plugin. You can also exit with the value `1` to have the same effect.
...
-   `options`
    ...
    a space separated list of options, enclosed in quotes. The following options are currently supported: `obsolete` to mark a chart as obsolete (Khulnasoft will hide it and delete it after some time), `store_first` to make Khulnasoft store the first collected value, assuming there was an invisible previous value set to zero (this is used by statsd charts - if the first data collected value of incremental dimensions is not zero based, unrealistic spikes will appear with this option set) and `hidden` to perform all operations on a chart, but do not offer it on dashboards (the chart will be send to external databases). `CHART` options have been added in Khulnasoft v1.7 and the `hidden` option was added in 1.10.

-   `plugin` and `module`

    both are just names that are used to let the user identify the plugin and the module that generated the chart. If `plugin` is unset or empty, Khulnasoft will automatically set the filename of the plugin that generated the chart. `module` has not default.
...
-   `options`
    ...
    a space separated list of options, enclosed in quotes. Options supported: `obsolete` to mark a dimension as obsolete (Khulnasoft will delete it after some time) and `hidden` to make this dimension hidden, it will take part in the calculations but will not be presented in the chart.
...
-   `LOCAL` or `CHART` to define the variable at the chart level. Use chart-local variables when the same variable may exist for different charts (i.e. Khulnasoft monitors 2 mysql servers, and you need to set the `max_connections` each server accepts). Using chart-local variables is the ideal to build alert templates.
...
The `value` is floating point (Khulnasoft used `long double`).

Variables are transferred to upstream Khulnasoft servers (streaming and database replication).
...
- `8`: This is a label defined using `khulnasoft` Agent-Cloud link.
...
The plugin can register functions to Khulnasoft, like this:
...
- Access is one of `any`, `member`, or `admin`:
  - `any` to offer the function to all users of Khulnasoft, even if they are not authenticated.
  - `member` to offer the function to all authenticated members of Khulnasoft.
...
Users can use a function to ask for more information from the collector. Khulnasoft maintains a registry of functions in 2 levels:

- per node
- per chart

Both node and chart functions are exactly the same, but chart functions allow Khulnasoft to relate functions with charts and therefore present a context-sensitive menu of functions related to the chart the user is using.

Users can get a list of all the registered functions using the `/api/v1/functions` endpoint of Khulnasoft and call functions using the `/api/v1/function` API call of Khulnasoft.
...
In this case, Khulnasoft will send:
...
Note 2: Khulnasoft always sends the function and its parameters enclosed in double quotes. If the function command and its parameters contain quotes, they are converted to single quotes.

The plugin is expected to parse and validate `name and parameters of the function as  one quotes parameter`. Khulnasoft allows the user interface to manipulate this string by appending more parameters.
...
Where:
  ...
  - `transaction_id` is the transaction id that Khulnasoft sent for this function execution
  - `http_response_code` is the http error code Khulnasoft should respond with, 200 is the "ok" response
...
To terminate the message, Khulnasoft seeks a line with just this:
...
After this line, Khulnasoft resumes processing collected metrics from the plugin.

The maximum uncompressed payload size Khulnasoft will accept is 100MB.

##### Functions cancellation

Khulnasoft is able to detect when a user made an API request, but abandoned it before it was completed. If this happens to an API called for a function served by the plugin, Khulnasoft will generate a `FUNCTION_CANCEL` request to let the plugin know that it can stop processing the query.
...
When a request takes too long to be processed, Khulnasoft allows the plugin to report progress to Khulnasoft, which in turn will report progress to the caller.
...
Khulnasoft supports two kinds of progress:
...
To accomplish this, when Khulnasoft receives a progress request by a user, it generates progress requests to the plugin, updating all the intermediate nodes to extend their timeout if necessary.
...
`CONFIG` commands sent from the plugin to Khulnasoft define dynamic configuration entities. These configurable entities are exposed to the user interface, allowing users to change configuration at runtime.

Dynamically configurations made this way are saved to disk by Khulnasoft and are replayed automatically when Khulnasoft or the plugin restarts.
...
> IMPORTANT:<br/>
> The plugin should blindly create, delete and update the status of its dynamic configuration entities, without any special logic applied to it. Khulnasoft needs to be updated of what is actually happening at the plugin. Keep in mind that creating dynamic configuration entities triggers responses from Khulnasoft, depending on its type and status. Re-creating a job, triggers the same responses every time, so make sure you create jobs only when you add jobs. 
...
- `status` can be:
  ...
  - `orphan`, the configuration is not claimed by any plugin. This is used internally by Khulnasoft to mark the configuration nodes available, for which there is no plugin related to them. Do not use in plugins directly.
...
- `path` is the absolute path of the configurable entity inside the tree of Khulnasoft configurations. Usually, this is should be `/collectors`.
...
- `supported_commands` is a space separated list of the following keywords, enclosed in single or double quotes. These commands are used by the user interface to determine the actions the users can take:
  - `schema`, to expose the JSON schema for the user interface. This is mandatory for all configurable entities. When `schema` requests are received, Khulnasoft will first attempt to load the schema from `/etc/khulnasoft/schema.d/` and `/var/lib/khulnasoft/conf.d/schema.d`. For jobs, it will serve the schema of their template. If no schema is found for the required `id`, the `schema` request will be forwarded to the plugin, which is expected to send back the relevant schema.
  ...
  - `enable` and `disable`, to receive user requests to enable and disable this entity. Adding only one of `enable` or `disable` to the supported commands, Khulnasoft will add both of them. The plugin should expose these commands on `templates` only when it wants to receive `enable` and `disable` commands for all the `jobs` of this `template`.
  - `restart`, to restart a job.
- `view permissions` and `edit permissions` are bitmaps of the Khulnasoft permission system to control access to the configuration. If set to zero, Khulnasoft will require a signed in user with view and edit permissions to the Khulnasoft's configuration system.

The plugin receives commands as if it had exposed a `FUNCTION` named `config`. Khulnasoft formats all these calls like this:

```
config id command
```

Where `id` is the unique id of the configurable entity and `command` is one of the supported commands the plugin sent to Khulnasoft.
...
Immediately after the plugin adds a configuration entity, if the commands `enable` and `disable` are supported by it, Khulnasoft will send either `enable` or `disable` for it, based on the last user action, which has been persisted to disk.
...
When responding to additions and updates, Khulnasoft uses the following success response codes to derive additional information:
...
-   `microseconds`
    ...
    data from the plugins to Khulnasoft via the pipe. This number improves
    accuracy significantly, since the plugin is able to calculate the
    duration between its iterations better than Khulnasoft.
...
it can issue a `FLUSH`. The `FLUSH` command will instruct Khulnasoft to ignore
...
follow these guidelines), will be disabled by Khulnasoft.

### collected values

Khulnasoft will collect any **signed** value in the 64bit range:
...
1.  **python**, use `python.d.plugin`, there are many examples in the [python.d
    directory](/docs/developer-and-contributor-corner/external-plugins/python.d.plugin)

    python is ideal for Khulnasoft plugins. It is a simple, yet powerful way to collect data, it has a very small memory footprint, although it is not the most CPU efficient way to do it.

2.  **BASH**, use `charts.d.plugin`, there are many examples in the [charts.d
    directory](/docs/developer-and-contributor-corner/external-plugins/charts.d.plugin)
...
3.  **C**
    ...
    Of course, C is the most efficient way of collecting data. This is why Khulnasoft itself is written in C.
...
```js
   ...
   while(true) {
       ...
       /* send the collected data to Khulnasoft */
...
```
   ...
   Khulnasoft interpolates values to second boundaries, so even if your plugin is not perfectly aligned it does not matter. Khulnasoft will find out. When your plugin works in increments of `update_every`, there will be no gaps in the charts due to the possible cumulative micro-delays in data collection. Gaps will only appear if the data collection is really delayed.

3.  If you are not sure of memory leaks, exit every one hour. Khulnasoft will re-start your process.
