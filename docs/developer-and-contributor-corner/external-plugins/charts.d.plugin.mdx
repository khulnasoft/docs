---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/collectors/charts.d.plugin/README.md"
sidebar_label: "charts.d.plugin"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner/External-plugins"
sidebar_position: "1607500"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/external-plugins/charts.d.plugin"
---

# charts.d.plugin

`charts.d.plugin` is a Khulnasoft external plugin. It is an **orchestrator** for data collection modules written in `BASH` v4+.

1. It runs as an independent process `ps fax` shows it
2. It is started and stopped automatically by Khulnasoft
3. It communicates with Khulnasoft via a unidirectional pipe (sending data to the `khulnasoft` daemon)
4. Supports any number of data collection **modules**

To better understand the guidelines and the API behind our External plugins, please have a look at the [Introduction to External plugins](/docs/developer-and-contributor-corner/external-plugins) prior to reading this page.
...
`charts.d.plugin` looks for scripts in `/usr/lib/khulnasoft/charts.d`.
The scripts should have the filename suffix: `.chart.sh`.

By default, `charts.d.plugin` is not included as part of the install when using [our official native DEB/RPM packages](/docs/khulnasoft-agent/installation/linux/native-linux-distribution-packages). You can install it by installing the `khulnasoft-plugin-chartsd` package.

## Configuration

`charts.d.plugin` itself can be [configured](/docs/khulnasoft-agent/configuration#edit-a-configuration-file-using-edit-config)using the configuration file `/etc/khulnasoft/charts.d.conf`. This file is also a BASH script.
...
1. The module script must be called `X.chart.sh` and placed in `/usr/libexec/khulnasoft/charts.d`.

2. If the module needs a configuration, it should be called `X.conf` and placed in `/etc/khulnasoft/charts.d`.  
    The configuration file `X.conf` is also a BASH script itself.  
    You can edit the default files supplied by Khulnasoft, by editing `/etc/khulnasoft/edit-config charts.d/X.conf`, where `X` is the name of the module.
...
4. The following functions must be defined:
    ...
    - `X_create()` - creates the Khulnasoft charts (commands `CHART` and `DIMENSION`).
...
The purpose of the BASH function `X_create()` is to create the charts and dimensions using the standard Khulnasoft
plugin guidelines.
...
`X_update()` will be called repeatedly every `X_update_every` seconds, to collect new values and send them to Khulnasoft,
following the Khulnasoft plugin guidelines.
...
sudo su -s /bin/sh khulnasoft

# run the plugin in debug mode
/usr/libexec/khulnasoft/plugins.d/charts.d.plugin debug 1 X Y Z
```
...
Keep in mind that if your configs are not in `/etc/khulnasoft`, you should do the following before running
`charts.d.plugin`:

```sh
export KHULNASOFT_USER_CONFIG_DIR="/path/to/etc/khulnasoft"
```

Also, remember that Khulnasoft runs `chart.d.plugin` as user `khulnasoft` (or any other user the `khulnasoft` process is configured to run as).
...
2. Create/edit the files `/etc/khulnasoft/charts.d.conf` and `/etc/khulnasoft/charts2.d.conf` and enable / disable the
    module you want each to run. Remember to set `enable_all_charts="no"` to both of them, and enable the individual
    modules for each.

3. link `/usr/libexec/khulnasoft/plugins.d/charts.d.plugin` to `/usr/libexec/khulnasoft/plugins.d/charts2.d.plugin`.
    Khulnasoft will spawn a new charts.d process.

Execute the above in this order, since Khulnasoft will (by default) attempt to start new plugins soon after they are
created in `/usr/libexec/khulnasoft/plugins.d/`.
