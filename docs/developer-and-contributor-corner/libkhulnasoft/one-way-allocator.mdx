---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/libkhulnasoft/onewayalloc/README.md"
sidebar_label: "One way allocator"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner/libkhulnasoft"
sidebar_position: "1612110"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/libkhulnasoft/one-way-allocator"
---
...
It has been designed to be used for khulnasoft context queries.

For khulnasoft to perform a context query, it builds a virtual chart, a chart that contains
...
1. The caller calls `ONEWAYALLOC *owa = onewayalloc_create(sizehint)` to create an OWA.
   ...
   No need to check for success or failure. As with `mallocz()` in khulnasoft, a `fatal()`
...
Khulnasoft was configured to use 24 web threads on the 24 core server we used.
...
For parallel workload, like the one executed by khulnasoft.com, `OWA` provides a 54% overall speed improvement (more than double the overall
