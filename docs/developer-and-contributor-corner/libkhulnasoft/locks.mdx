---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/libkhulnasoft/locks/README.md"
sidebar_label: "Locks"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner/libkhulnasoft"
sidebar_position: "1612090"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/libkhulnasoft/locks"
---

# Locks

## How to trace khulnasoft locks

To enable tracing rwlocks in khulnasoft, compile khulnasoft by setting `CFLAGS="-DKHULNASOFT_TRACE_RWLOCKS=1"`, like this:

```
CFLAGS="-O1 -ggdb -DKHULNASOFT_TRACE_RWLOCKS=1"  ./khulnasoft-installer.sh
```

During compilation, the compiler will log:

```
libkhulnasoft/locks/locks.c:105:2: warning: #warning KHULNASOFT_TRACE_RWLOCKS ENABLED - EXPECT A LOT OF OUTPUT [-Wcpp]
  105 | #warning KHULNASOFT_TRACE_RWLOCKS ENABLED - EXPECT A LOT OF OUTPUT
      |  ^~~~~~~
```

Once compiled, khulnasoft will do the following:

Every call to `khulnasoft_rwlock_*()` is now measured in time.
...
The time can be changed by setting this `-DKHULNASOFT_TRACE_RWLOCKS_WAIT_TIME_TO_IGNORE_USEC=20` (or whatever number) to the CFLAGS.
...
The time can be changed by setting this `-DKHULNASOFT_TRACE_RWLOCKS_HOLD_TIME_TO_IGNORE_USEC=20000` (or whatever number) to the CFLAGS.
...
When compiled with `-DKHULNASOFT_TRACE_RWLOCKS_LOG_NESTED=1` the library will also detect nested read locks and print them like this:
