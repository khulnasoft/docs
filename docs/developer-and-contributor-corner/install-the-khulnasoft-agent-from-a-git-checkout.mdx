---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/packaging/installer/methods/manual.md"
sidebar_label: "Install the Khulnasoft Agent from a Git checkout"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner"
description: "Use the Khulnasoft Agent source code from GitHub, plus helper scripts to set up your system, to install Khulnasoft without packages or binaries."
sidebar_position: "1618000"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/install-the-khulnasoft-agent-from-a-git-checkout"
---

# Install Khulnasoft on Linux from a Git checkout

To install the latest git version of Khulnasoft, please follow these 2 steps:

1. [Prepare your system](#prepare-your-system)

    Install the required packages on your system.

2. [Install Khulnasoft](#install-khulnasoft)

    Download and install Khulnasoft. You can also update it the same way.
...
should be installed on your system to build and run Khulnasoft. It supports a large variety of major Linux distributions
and other operating systems and is regularly tested. You can find this tool [here](https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/packaging/installer/install-required-packages.sh) or run it directly with `bash <(curl -sSL https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/packaging/installer/install-required-packages.sh)`. Otherwise read on for how to get requires packages manually:
...
- **SLE12** Must have your system registered with SUSE Customer Center or have the DVD. See
    [#1162](https://github.com/khulnasoft/khulnasoft/issues/1162)

Install the packages for having a **basic Khulnasoft installation** (system monitoring and many applications, without  `mysql` / `mariadb`, `named`, hardware sensors and `SNMP`):

```sh
curl -Ss 'https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/packaging/installer/install-required-packages.sh' >/tmp/install-required-packages.sh && bash /tmp/install-required-packages.sh -i khulnasoft
```

Install all the required packages for **monitoring everything Khulnasoft can monitor**:

```sh
curl -Ss 'https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/packaging/installer/install-required-packages.sh' >/tmp/install-required-packages.sh && bash /tmp/install-required-packages.sh -i khulnasoft-all
```

If the above do not work for you, please [open a github
issue](https://github.com/khulnasoft/khulnasoft/issues/new?title=packages%20installer%20failed&labels=installation%20help&body=The%20experimental%20packages%20installer%20failed.%0A%0AThis%20is%20what%20it%20says:%0A%0A%60%60%60txt%0A%0Aplease%20paste%20your%20screen%20here%0A%0A%60%60%60)
with a copy of the message you get on screen. We are trying to make it work everywhere (this is also why the script
[reports back](https://github.com/khulnasoft/khulnasoft/issues/2054) success or failure for all its runs).
...
Once Khulnasoft is compiled, to run it the following packages are required (already installed using the above commands):

|  package  | description                                                                           |
|:---------:|---------------------------------------------------------------------------------------|
| `libuuid` | part of `util-linux` for GUIDs management                                             |
|  `zlib`   | gzip compression for the internal Khulnasoft web server                                  |
|  `libuv`  | Multi-platform support library with a focus on asynchronous I/O, version 1 or greater |

_Khulnasoft will fail to start without the above._

Khulnasoft plugins and various aspects of Khulnasoft can be enabled or benefit when these are installed (they are optional):
...
_Khulnasoft will greatly benefit if you have the above packages installed, but it will still work without them._

Khulnasoft DB engine can be enabled when these are installed (they are optional):
...
Khulnasoft Cloud support may require the following packages to be installed:
...
| `openssl`  | Needed to secure communications with the Khulnasoft Cloud                                                                               |
...
On CentOS / RHEL 6.x, many of the dependencies for Khulnasoft are only
...
Do this to install and run Khulnasoft:

```sh
# download it - the directory 'khulnasoft' will be created
git clone https://github.com/khulnasoft/khulnasoft.git --depth=100 --recursive
cd khulnasoft

# run script with root privileges to build, install, start Khulnasoft
./khulnasoft-installer.sh
```
...
- If you don't want to install it on the default directories, you can run the installer like this: `./khulnasoft-installer.sh --install-prefix /opt`. This one will install Khulnasoft in `/opt/khulnasoft`.
...
`khulnasoft-installer.sh` accepts a few parameters to customize your installation:

- `--dont-wait`: Enable automated installs by not prompting for permission to install any required packages.
- `--dont-start-it`: Prevent the installer from starting Khulnasoft automatically.
- `--stable-channel`: Automatically update only on the release of new major versions.
- `--nightly-channel`: Automatically update on every new nightly build.
- `--disable-telemetry`: Opt-out of [anonymous statistics](/docs/khulnasoft-agent/anonymous-telemetry-events) we use to make
    Khulnasoft better.
...
- `--local-files`: Used for [offline installations](/docs/khulnasoft-agent/installation/linux/offline-systems). Pass four file paths: the Khulnasoft
    tarball, the checksum file, the go.d plugin tarball, and the go.d plugin config tarball, to force kickstart run the
    process using those files. This option conflicts with the `--stable-channel` option. If you set this _and_
    `--stable-channel`, Khulnasoft will use the local files.

### Connect node to Khulnasoft Cloud during installation

Unlike the [`kickstart.sh`](/docs/khulnasoft-agent/installation/linux), the `khulnasoft-installer.sh` script does
not allow you to automatically [connect](/docs/khulnasoft-cloud/connect-agent-to-cloud) your node to Khulnasoft Cloud immediately after installation.

See the [connect to Khulnasoft Cloud](/docs/khulnasoft-cloud/connect-agent-to-cloud) doc for details on connecting a node with a manual installation of Khulnasoft.
...
If the installation fails with errors like `/bin/ld: externaldeps/libwebsockets/libwebsockets.a(context.c.o): relocation R_X86_64_32 against '.rodata.str1.1' can not be used when making a PIE object; recompile with -fPIC`, and you are trying to build with `clang` on Linux, you will need to build Khulnasoft using GCC to get a fully functional install.

In most cases, you can do this by running `CC=gcc ./khulnasoft-installer.sh`.

### Perform a cleanup in your khulnasoft repo

The Khulnasoft repo consist of the main git tree and it's submodules. Either working on a fork or on the main repo you need to make sure that there
