---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/web/api/formatters/csv/README.md"
sidebar_label: "CSV formatter"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner/REST API/Formatters"
sidebar_position: "1603010"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/rest-api/formatters/csv-formatter"
---
...
Khulnasoft always returns bandwidth in `kilobits`.

```bash
# curl -Ss 'https://registry.my-khulnasoft.io/api/v1/data?chart=system.net&format=csv&after=-3600&group=sum&points=6&options=abs'
time,received,sent
...
Khulnasoft always returns memory in `MB`.

```bash
# curl -Ss 'https://registry.my-khulnasoft.io/api/v1/data?chart=apps.mem&format=tsv&after=-3600&group=max&points=2&options=nonzero,seconds&dimensions=sql,cron'
time	sql	cron
...
Khulnasoft always returns CPU utilization as `%`.

```bash
# curl -Ss 'https://registry.my-khulnasoft.io/api/v1/data?chart=system.cpu&format=html&after=-4&options=nonzero'
<html>
<center>
...
Khulnasoft always returns bandwidth rates in `kilobits/s`.

```bash
# curl -Ss 'https://registry.my-khulnasoft.io/api/v1/data?chart=mysql_local.net&format=csvjsonarray&after=-3600&points=6&group=average&options=abs,ms'
[
["time","in","out"],
...
```bash
# curl -Ss 'https://registry.my-khulnasoft.io/api/v1/data?chart=system.forks&format=markdown&after=-600&points=10&group=sum'
time               | started
:---:              |:---:
