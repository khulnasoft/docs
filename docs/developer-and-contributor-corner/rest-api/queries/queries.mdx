---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/web/api/queries/README.md"
sidebar_label: "Queries"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner/REST API/Queries"
sidebar_position: "1602500"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/rest-api/queries/queries"
---

# Database queries/lookup

This document explains in detail the options available to retrieve data from the Khulnasoft timeseries database in order to configure alerts, create badges or 
create custom charts.

The Khulnasoft database can be queried with the `/api/v1/data` and `/api/v1/badge.svg` REST API methods. The database is also queried from the `lookup` line 
in an [alert configuration](/docs/alerts-&-notifications/alert-configuration-reference).
...
|`points`|no|The number of points to be returned. Khulnasoft can reduce number of points by applying query grouping methods. If not given, the result will have the same granularity as the database (although this relates to `gtime`).|
...
-   The caller requests a specific number of `points` to be returned.
    ...
    This feature is used by Khulnasoft dashboards when you zoom-out the charts.
...
Alignment is a very important aspect of Khulnasoft queries. Without it, the animated
...
The engine does not evaluate dimensions that do not match the [simple pattern](/docs/developer-and-contributor-corner/libkhulnasoft/simple-patterns)
...
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=min&after=-60&label=min&value_color=blue) finds the minimum value
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=max&after=-60&label=max&value_color=lightblue) finds the maximum value
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=average&after=-60&label=average&value_color=yellow) finds the average value
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=sum&units=kilobits&after=-60&label=sum&value_color=orange) adds all the values and returns the sum
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=median&after=-60&label=median&value_color=red) sorts the values and returns the value in the middle of the list
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=stddev&after=-60&label=stddev&value_color=green) finds the standard deviation of the values
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=cv&after=-60&label=cv&units=pcent&value_color=yellow) finds the relative standard deviation (coefficient of variation) of the values
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=ses&after=-60&label=ses&value_color=brown) finds the exponential weighted moving average of the values
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=des&after=-60&label=des&value_color=blue) applies Holt-Winters double exponential smoothing
-   ![](https://registry.my-khulnasoft.io/api/v1/badge.svg?chart=net.eth0&options=unaligned&dimensions=received&group=incremental_sum&after=-60&label=incremental_sum&value_color=red) finds the difference of the last vs the first value

The examples shown above show live information from the `received` traffic on the `eth0` interface of the global Khulnasoft Registry.
...
```
https://registry.my-khulnasoft.io/api/v1/data?chart=net.eth0&options=unaligned&dimensions=received&group=sum&units=kilobits&after=-60&label=sum&points=1
```
...
When Khulnasoft is reducing metrics, it tries to return always the same boundaries. So, if we want 10s averages, it will always return points starting at a `unix timestamp % 10 = 0`.
...
At 00:04 you ask for 2 points for 4 seconds in the past. So `group = 2`. Khulnasoft would return:
...
For this reason, Khulnasoft always aligns the data it returns to the `group`.

When you request `points=1`, Khulnasoft understands that you need 1 point for the whole database, so `group = 3600`. Then it tries to find the starting point which would be `timestamp % 3600 = 0` Within a database of 3600 seconds, there is one such point for sure. Then it tries to find the average of 3600 points. But, most probably it will not find 3600 of them (for just 1 out of 3600 seconds this query will return something).
...
<http://khulnasoft.firehol.org/api/v1/data?chart=system.cpu&points=1&after=-10&options=seconds>
