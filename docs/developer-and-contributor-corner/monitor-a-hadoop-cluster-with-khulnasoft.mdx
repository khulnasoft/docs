---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/developer-and-contributor-corner/monitor-hadoop-cluster.md"
sidebar_label: "Monitor a Hadoop cluster with Khulnasoft"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner"
sidebar_position: "1614100"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/monitor-a-hadoop-cluster-with-khulnasoft"
---

# Monitor a Hadoop cluster with Khulnasoft
...
Khulnasoft comes with built-in and pre-configured support for monitoring both HDFS and Zookeeper.
...
As with all data sources, Khulnasoft can auto-detect HDFS and Zookeeper nodes if you installed them using the standard
installation procedure.

For Khulnasoft to collect HDFS metrics, it needs to be able to access the node's `/jmx` endpoint. You can test whether an
...
If Khulnasoft can't access the `/jmx` endpoint for either a NameNode or DataNode, it will not be able to auto-detect and
...
To configure Khulnasoft's HDFS module, navigate to your Khulnasoft directory (typically at `/etc/khulnasoft/`) and use
...
At this point, Khulnasoft should be configured to collect metrics from your HDFS servers. Let's move on to Zookeeper.
...
Finally, [restart Khulnasoft](/docs/khulnasoft-agent/maintenance/service-control).

```sh
sudo systemctl restart khulnasoft
```

Upon restart, Khulnasoft should recognize your HDFS/Zookeeper servers, enable the HDFS and Zookeeper modules, and begin
showing real-time metrics for both in your Khulnasoft dashboard. 🎉

## Configuring HDFS and Zookeeper alerts

The Khulnasoft community helped us create sane defaults for alerts related to both HDFS and Zookeeper. You may want to
investigate these to ensure they work well with your Hadoop implementation.

- [HDFS alerts](https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/src/health/health.d/hdfs.conf)

You can also access/edit these files directly with `edit-config`:

```bash
sudo /etc/khulnasoft/edit-config health.d/hdfs.conf
sudo /etc/khulnasoft/edit-config health.d/zookeeper.conf
```

For more information about editing the defaults or writing new alert entities, see our [health monitoring documentation](/docs/alerts-&-notifications).
