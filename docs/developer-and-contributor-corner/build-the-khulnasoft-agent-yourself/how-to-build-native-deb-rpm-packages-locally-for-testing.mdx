---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/packaging/building-native-packages-locally.md"
...
sidebar_position: "1616040"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/build-the-khulnasoft-agent-yourself/how-to-build-native-(deb/rpm)-packages-locally-for-testing"
---
...
```bash
docker run -it --rm -e VERSION=0.1 -v $PWD:/khulnasoft khulnasoft/package-builders:<tag>
```

or

```bash
podman run -it --rm -e VERSION=0.1 -v $PWD:/khulnasoft khulnasoft/package-builders:<tag>
```
...
[khulnasoft/package-builders](https://hub.docker.com/r/khulnasoft/package-builders/tags) for all available tags.
...
The environments used for building our packages are fully self-contained Docker images built from [Dockerfiles](https://github.com/khulnasoft/helper-images/tree/master/package-builders)
These are published on Docker
Hub with the image name `khulnasoft/package-builders`, and tagged using the name and version of the distribution
...
* It expects the source tree it should build from to be located at `/khulnasoft`, and expects that said source tree
...
On DEB systems (Ubuntu and Debian), the build directory inside the container is located at `/usr/src/khulnasoft`
