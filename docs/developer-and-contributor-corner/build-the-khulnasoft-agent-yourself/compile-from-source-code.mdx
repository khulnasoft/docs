---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/packaging/installer/methods/source.md"
...
description: "Package maintainers and power users may be interested in manually building Khulnasoft from source without using any of our installation scripts."
sidebar_position: "1616020"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/build-the-khulnasoft-agent-yourself/compile-from-source-code"
---

# Manually build Khulnasoft from source
...
to follow [our guide for manually installing Khulnasoft from a git
checkout](/docs/developer-and-contributor-corner/install-the-khulnasoft-agent-from-a-git-checkout) instead.

## Required dependencies

At a bare minimum, Khulnasoft requires the following libraries and tools
...
- Khulnasoft Cloud support:
  - A working internet connection
  - OpenSSL 1.0.2 or newer _or_ LibreSSL 3.0.0 or newer.
  - protobuf (Google Protocol Buffers) and protoc compiler. If protobuf is not available on the system,
        CMake can be instructed to fetch and build a usable version for Khulnasoft.
- Khulnasoft Go collectors:
  - Go 1.21 or newer

## Preparing the source tree

Khulnasoft uses Git submodules for some of it’s components, which must be fetched prior to building Khulnasoft. If you
are using a source tarball published by the Khulnasoft project, then these are included. If you are using a checkout
...
Once the source tree has been prepared, Khulnasoft is ready to be configured
and built. Khulnasoft uses CMake for configuration, and strongly prefers
the use of an external build directory. To configure and build Khulnasoft:
...
Khulnasoft’s CMake build infrastructure intentionally does very little auto-detection, and requires most components
to be explicitly enabled or disabled. A full list of available configuration options for a given version of Khulnasoft,
with help descriptions, can be seen by running `cmake -LH` in the source tree.

### Using Clang

Khulnasoft is primarily developed using GCC, but in most cases we also
...
Khulnasoft's build system currently does not reliably support building
...
Khulnasoft trying to build against headers for one version but link to a
different version.

## Additional components

A full featured install of Khulnasoft requires some additional components
which must be built and installed separately from the main Khulnasoft
agent. All of these should be handled _after_ installing Khulnasoft itself.

### eBPF collector

On Linux systems, Khulnasoft has support for using the kernel's eBPF
...
a separate repository from the core Khulnasoft Agent. You can either install
...
1. Verify the release version that Khulnasoft expects to be used by checking
    the contents of `packaging/ebpf.version` in your Khulnasoft sources.
2. Go to <https://github.com/khulnasoft/kernel-collector/releases>, select the
    required release, and download the `khulnasoft-kernel-collector-*.tar.xz`
...
3. Extract the contents of the archive to a temporary location, and then
    ...
    directory to `/usr/libexec/khulnasoft/plugins.d` or the equivalent location
    for your build of Khulnasoft.
...
repository](https://github.com/khulnasoft/kernel-collector/#readme),
