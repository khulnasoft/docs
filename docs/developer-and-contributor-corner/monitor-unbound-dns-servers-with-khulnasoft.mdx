---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/docs/developer-and-contributor-corner/collect-unbound-metrics.md"
sidebar_label: "Monitor Unbound DNS servers with Khulnasoft"
docs_status: "Published"
docs_rel_path: "Developer and Contributor Corner"
sidebar_position: "1614080"
docs_link: "https://docs.khulnasoft.com/docs/developer-and-contributor-corner/monitor-unbound-dns-servers-with-khulnasoft"
---

# Monitor Unbound DNS servers with Khulnasoft

[Unbound](https://nlnetlabs.nl/projects/unbound/about/) is a "validating, recursive, caching DNS resolver" from NLNet
Labs. In v1.19 of Khulnasoft, we release a completely refactored collector for collecting real-time metrics from Unbound
servers and displaying them in Khulnasoft dashboards.
...
As with all data sources, Khulnasoft can auto-detect Unbound servers if you installed them using the standard installation
procedure.
...
Next, make your `unbound.conf`, `unbound_control.key`, and `unbound_control.pem` files readable by Khulnasoft using [access
control lists](https://wiki.archlinux.org/index.php/Access_Control_Lists) (ACL).

```bash
sudo setfacl -m user:khulnasoft:r unbound.conf
sudo setfacl -m user:khulnasoft:r unbound_control.key
sudo setfacl -m user:khulnasoft:r unbound_control.pem
```
...
You may not need to do any more configuration to have Khulnasoft collect your Unbound metrics.

If you followed the steps above to enable `remote-control` and make your Unbound files readable by Khulnasoft, that should
be enough. Restart Khulnasoft with `sudo systemctl restart khulnasoft`, or the appropriate method for your system. You should see Unbound metrics in your Khulnasoft dashboard!
...
To configure Khulnasoft's Unbound collector module, navigate to your Khulnasoft configuration directory (typically at
`/etc/khulnasoft/`) and use `edit-config` to initialize and edit your Unbound configuration file.

```bash
cd /etc/khulnasoft/ # Replace with your Khulnasoft configuration directory, if not /etc/khulnasoft/
...
Khulnasoft will attempt to read `unbound.conf` to get the appropriate `address`, `cumulative`, `use_tls`, `tls_cert`, and
`tls_key` parameters.

Restart Khulnasoft with `sudo systemctl restart khulnasoft`, or the appropriate method for your system.
...
file](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/plugin/go.d/config/go.d/unbound.conf) contains a few useful examples:
...
file](https://github.com/khulnasoft/khulnasoft/blob/master/src/go/plugin/go.d/config/go.d/unbound.conf).
...
issue](https://github.com/khulnasoft/khulnasoft/issues/new?assignees=&labels=bug%2Cneeds+triage&template=BUG_REPORT.yml) with your
thoughts.
