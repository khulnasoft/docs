---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/collectors/windows-events.plugin/README.md"
...
docs_link: "https://docs.khulnasoft.com/docs/logs/windows-event-logs/windows-events-plugin-reference"
---
...
`windows-events.plugin` is a Khulnasoft Function Plugin.
...
2. **Version**  
   ...
   Khulnasoft adds this field automatically when it is not zero.
...
3. **Level**  
   ...
   Khulnasoft provides 2 fields: `Level` and `LevelID` for the text and numeric representation of it.

4. **Opcode**  
   The action or state within a provider when the event was logged.
   
   Khulnasoft provides 2 fields: `Opcode` and `OpcodeID` for the text and numeric representation of it.
...
6. **Task**  
   ...
   Khulnasoft provides 2 fields: `Task` and `TaskID` for the text and numeric representation of it.
...
10. **Keywords**  
    ...
    Khulnasoft provides 2 fields: `Keywords` and `keywordsID` for the text and numeric representation of it.
...
11. **Provider**  
    ...
    Khulnasoft provides 2 fields: `Provider` and `ProviderGUID` for its name and GUID of it.
...
15. **User**
    ...
    Khulnasoft provides 3 fields: `UserAccount`, `UserDomain` and `UserSID`.
...
when there are more than few thousand log entries to explore. So, Khulnasoft presents them
with lazy loading.

This prevents Khulnasoft for offering filtering for user-defined fields, although Khulnasoft does support
...
This Khulnasoft plugin does not require any specific configuration. It automatically detects available event logs
on the system.
...
mechanisms, and then install Khulnasoft on this events centralization server to explore the events of all your
infrastructure.
...
Yes. When your nodes are connected to a Khulnasoft parent, all their functions are available via the parent's UI.
So, from the parent UI, you can access the functions of all your nodes.

Keep in mind that to protect your privacy, in order to access Khulnasoft functions, you need a free Khulnasoft Cloud
account.

### Is any of my data exposed to Khulnasoft Cloud from this plugin?

No. When you access the Agent directly, none of your data passes through Khulnasoft Cloud. You need a free Khulnasoft
Cloud account only to verify your identity and enable the use of Khulnasoft Functions. Once this is done, all the
data flow directly from your Khulnasoft Agent to your web browser.

When you access Khulnasoft via https://app.khulnasoft.com, your data travel via Khulnasoft Cloud, but they are not stored
in Khulnasoft Cloud. This is to allow you access your Khulnasoft Agents from anywhere. All communication from/to
Khulnasoft Cloud is encrypted.
