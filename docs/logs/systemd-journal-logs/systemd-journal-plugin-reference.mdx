---
custom_edit_url: "https://github.com/khulnasoft/khulnasoft/edit/master/src/collectors/systemd-journal.plugin/README.md"
...
docs_link: "https://docs.khulnasoft.com/docs/logs/systemd-journal-logs/systemd-journal-plugin-reference"
---
...
The `systemd` journal plugin by Khulnasoft makes viewing, exploring and analyzing `systemd` journal logs simple and
efficient.
...
![image](https://github.com/khulnasoft/khulnasoft/assets/2662304/691b7470-ec56-430c-8b81-0c9e49012679)
...
`systemd-journal.plugin` is a Khulnasoft Function Plugin.

To protect your privacy, as with all Khulnasoft Functions, a free Khulnasoft Cloud user account is required to access it.
For more information, check [this discussion](https://github.com/khulnasoft/khulnasoft/discussions/16136).
...
- Khulnasoft versions prior to 1.44 shipped in a docker container do not include this plugin.
  The problem is that `libsystemd` is not available in Alpine Linux (there is a `libsystemd`, but it is a dummy that
  returns failure on all calls). Starting with Khulnasoft version 1.44, Khulnasoft containers use a Debian base image
  making this plugin available when Khulnasoft is running in a container.
- For the same reason (lack of `systemd` support for Alpine Linux), the plugin is not available on `static` builds of
  Khulnasoft (which are based on `muslc`, not `glibc`). If your Khulnasoft is installed in `/opt/khulnasoft` you most likely have
  a static build of Khulnasoft.
...
![journal-sources](https://github.com/khulnasoft/khulnasoft/assets/2662304/28e63a3e-6809-4586-b3b0-80755f340e31)
...
Unlike `journalctl`, the Khulnasoft plugin allows viewing, exploring and querying the journal files of **all users**.
...
Usually `remote` journals are named by the IP of the server sending these logs. The Khulnasoft plugin automatically
...
![image](https://github.com/khulnasoft/khulnasoft/assets/2662304/cd75fb55-6821-43d4-a2aa-033792c7f7ac)
...
![image](https://github.com/khulnasoft/khulnasoft/assets/2662304/3207794c-a61b-444c-8ffe-6c07cbc90ae2)
...
![image](https://github.com/khulnasoft/khulnasoft/assets/2662304/ac710d46-07c2-487b-8ce3-e7f767b9ae0f)
...
When "full data queries" is on, Khulnasoft is applying all filtering to the data (not `libsystemd`), but this means
...
![image](https://github.com/khulnasoft/khulnasoft/assets/2662304/d3dcb1d1-daf4-49cf-9663-91b5b3099c2d)

## PLAY mode

The plugin supports PLAY mode to continuously update the screen with new log entries found in the journal files.
Hit the "play" button at the top of the Khulnasoft dashboard screen.
...
Readers (like this Khulnasoft plugin), open the journal files and `libsystemd`, behind the scenes, maps regions
...
At the time of this writing, this Khulnasoft plugin is about 25–30 times faster than `journalctl` on queries that access
...
However, even after these patches are merged, `journalctl` will still be 2x slower than this Khulnasoft plugin,
...
the Khulnasoft plugin queries each file individually, and then it merges the results to be returned.
This is transparent, thanks to the `facets` library in `libkhulnasoft` that handles on-the-fly indexing, filtering,
...
This Khulnasoft plugin does not require any configuration or maintenance.

## FAQ

### Can I use this plugin on journal centralization servers?

Yes. You can centralize your logs using `systemd-journal-remote`, and then install Khulnasoft
...
### Can I use this plugin from a parent Khulnasoft?

Yes. When your nodes are connected to a Khulnasoft parent, all their functions are available
via the parent's UI. So, from the parent UI, you can access the functions of all your nodes.

Keep in mind that to protect your privacy, to access Khulnasoft functions, you need a
free Khulnasoft Cloud account.

### Is any of my data exposed to Khulnasoft Cloud from this plugin?

No. When you access the Agent directly, none of your data passes through Khulnasoft Cloud.
You need a free Khulnasoft Cloud account only to verify your identity and enable the use of
Khulnasoft Functions. Once this is done, all the data flow directly from your Khulnasoft Agent
to your web browser.

Also check [this discussion](https://github.com/khulnasoft/khulnasoft/discussions/16136).

When you access Khulnasoft via `https://app.khulnasoft.com`, your data travel via Khulnasoft Cloud,
but they are not stored in Khulnasoft Cloud. This is to allow you to access your Khulnasoft Agents from
anywhere. All communication from/to Khulnasoft Cloud is encrypted.
...
### I centralize my logs with Loki. Why use Khulnasoft for my journals?
...
by centralizing your system logs and the use of this Khulnasoft plugin, is unparalleled.
...
If you want to set up your own passive journal centralization setup without encryption, [check out guide on it](/docs/observability-centralization-points/logs-centralization-points-with-systemd-journald/passive-journal-centralization-without-encryption).

#### _passive_ journal centralization with encryption using self-signed certificates

If you want to set up your own passive journal centralization setup using self-signed certificates for encryption, [check out guide on it](https://github.com/khulnasoft/khulnasoft/blob/master/docs/observability-centralization-points/logs-centralization-points-with-systemd-journald).
