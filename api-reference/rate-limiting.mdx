...
<Warning>
  It’s important to note that this feature is still in beta release, and is not activated for Teleflow Cloud. However, self-hosting users can enable it immediately via the `IS_REQUEST_RATE_LIMITING_ENABLED` flag.
