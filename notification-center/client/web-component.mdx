...
The Notification Center Web Component is a custom element that can be used in any web application. It is built by wrapping the React components and consists from its core the three pieces:  `TeleflowProvider`,  `PopoverNotificationCenter`, and  `NotificationBell` combined together into one.

## ESM module[](https://docs.teleflow.khulnasoft.com/notification-center/web-component#esm-module)

Make sure that you have installed the `@teleflow/notification-center` package:
...
```tsx
import { NotificationCenterWebComponent } from "@teleflow/notification-center";
...
[CodeSandBox Link 🔗](https://codesandbox.io/s/teleflow-web-component-esm-rltcr5)
...
[CodeSandBox Link 🔗](https://codesandbox.io/s/teleflow-web-component-bundled-rgrj5r?file=/index.html)
...
| -------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| backendUrl                 | string                                                                                       | Custom API location in case of self-hosted version of Teleflow.                                                                                                                                                                                                                                                  |
| socketUrl                  | string                                                                                       | Custom WebSocket Service location in case of self-hosted version of Teleflow.                                                                                                                                                                                                                                    |
| subscriberId               | string                                                                                       | The subscriberId is a custom identifier used when identifying your users within the Teleflow platform.                                                                                                                                                                                                           |
| applicationIdentifier      | string                                                                                       | Is a public identifier used to identify your application, can be found in the Teleflow Admin panel.                                                                                                                                                                                                              |
...
| theme                      | [ITeleflowTheme](/notification-center/client/react/api-reference#the-theme-interface)            | The object defining the the light/dark styles of the Notification Center component. We discourage you to use this prop to do the styling, instead it's recommended to use the styles property.                                                                                                               |
