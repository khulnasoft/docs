...
The TeleflowProvider is the root component of the React Notification Center package. It is used to wrap the components code and provide the notifications feed context to all its children.
...
| ----------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| backendUrl              | string (optional)                                                              | The custom backend URL, your own deployed instance of the API. The default is https://api.teleflow.khulnasoft.com.app/.                                                                                                                                                                                                                                                                         |
| socketUrl               | string (optional)                                                              | The custom socket URL, your own deployed instance of the WS app. The default is https://ws.teleflow.khulnasoft.com/.                                                                                                                                                                                                                                                                            |
| applicationIdentifier   | string                                                                         | The app id is taken from the Teleflow Settings -> API Keys tab.                                                                                                                                                                                                                                                                                                                     |
...
```typescript
interface ITeleflowProviderProps {
  backendUrl?: string;
  socketUrl?: string;
...
To use the Styles Interface, you need to identify the property you need to modify to achieve your desired look and pass it as an object in the `styles` prop. 
  ...
  <br/>
You can also check out our [docs on customizing styles](https://docs.teleflow.khulnasoft.com/notification-center/client/react/api-reference#styles-interface) to see how to define styles for various components that make up the notification center.
  <br/>
...
<Note>Please have a look at our [docs on customizing styles](https://docs.teleflow.khulnasoft.com/notification-center/client/react/api-reference#styles-interface) to see how to define styles for various components of the notification center.</Note>
</Accordion>
...
| theme                      | [ITeleflowTheme](#the-theme-interface) (optional)   | The theme object allowing you to override light and dark colors of the UI components. Deprecated for styling, please use the [styles](#styles-interface) prop on the TeleflowProvider.                           |
| tabs                       | [ITab](#the-tab-interface)(optional)            | Allows to define separate UI tabs for the notifications feed. The array of connection objects between the feed tab and [stores](#store-interface) (that you define on the TeleflowProvider) and feed identifier. |
...
This hook that allows you to get the notifications feed data and unseen notifications count. The notifications are fetched only when the `fetchingStrategy.fetchNotifications` is set to true interface, check the `initialFetchingStrategy` prop on the [TeleflowProvider](#teleflowprovider) component.
...
| stores | [IStore[]](#store-interface) | array of [IStore](#store-interface) passed to the [TeleflowProvider](#teleflowprovider). |
...
`useTeleflowContext` hook is used to get the reference to the [TeleflowProvider](#teleflowprovider) context.

```typescript
const result = useTeleflowContext();
```

`useTeleflowContext` hook return interface

```typescript
interface ITeleflowProviderContext {
  backendUrl?: string;
  subscriberId?: string;
  applicationIdentifier?: string;
...
`useTeleflowTheme` hook can be used to get the reference to the [theme](#the-theme-interface) object.

```typescript
const result = useTeleflowTheme();
```

`useTeleflowTheme` hook return interface

```typescript
interface ITeleflowThemeProviderContext {
  theme: ITeleflowTheme;
  common: ICommonTheme;
  colorScheme: ColorScheme;
}
...
| ------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| \_id                      | string                                    | A unique Teleflow message identifier                                                                     |
...
| templateIdentifier        | string                                    | A unique Teleflow workflow identifier                                                                    |
...
| subscriberId | string                            | identifier of the subscriber entity in the Teleflow Web Dashboard        |
