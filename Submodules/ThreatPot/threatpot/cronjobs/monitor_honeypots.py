...
from threatpot.cronjobs.base import Cronjob
from threatpot.cronjobs.honeypots import Honeypot
...
class MonitorHoneypots(Cronjob):
    ...
    def run(self):
        for honeypot_to_monitor in self.honeypots_to_monitor:
            ...
            if not hits:
                self.log.warning(f"no logs available for the Honeypot {honeypot_to_monitor.name}." f" Something could be wrong in the CyberPot cluster")
