...
cleanup() {
  ...
  echo
  if [ "${CYBERPOT_OSTYPE}" = "linux" ];
    then
      echo "## ... removing firewall rules."
      /opt/cyberpot/bin/rules.sh ${COMPOSE} unset
      echo
      if [ "${CYBERPOT_BLACKHOLE}" == "ENABLED" ] && [ -f "/etc/blackhole/mass_scanner.txt" ];
        then
          echo "## ... removing Blackhole routes."
          /opt/cyberpot/bin/blackhole.sh del
          echo
      fi
  fi
  kill -TERM "$PID"
...
check_var() {
    ...
    if [[ -z "$var_value" ]];
      then
        echo "# Error: $var_name is not set or empty. Please check CyberPot .env config."
        echo
        echo "# Aborting"
        exit 1
    fi
...
check_safety() {
    ...
    if [[ $var_value =~ [^a-zA-Z0-9_/.:-] ]];
      then
        echo "# Error: Unsafe characters detected in $var_name. Please check CyberPot .env config."
        echo
        echo "# Aborting"
        exit 1
    fi
      ...
      do
        if [[ $i =~ ^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$ ]];
          then
	        ...
	        echo "$i is not a valid base64 string. Please check CyberPot .env config."
	        echo
	        echo "# Aborting"
	        exit 1
	    fi
...
validate_format() {
    ...
    case "$var_name" in
        CYBERPOT_BLACKHOLE|CYBERPOT_PERSISTENCE|CYBERPOT_ATTACKMAP_TEXT)
            if ! [[ $var_value =~ ^(ENABLED|DISABLED|on|off|true|false)$ ]];
              then
                echo "# Error: Invalid value for $var_name. Expected ENABLED/DISABLED, on/off, true/false. Please check CyberPot .env config."
		        echo
		        echo "# Aborting"
                exit 1
            fi
...
validate_ip_or_domain() {
    ...
    # Check if CYBERPOT_HIVE_IP matches IPv4 or domain name
    if [[ ${myCHECK} =~ $ipv4Regex ]]; then
        ...
        echo "# Error: $myCHECK is not a valid IPv4 address or domain name. Please check CyberPot .env config."
        echo
        echo "# Aborting"
        exit 1
    fi
...
create_web_users() {
    echo
    echo "# Creating passwd files based on CyberPot .env config ..."
...
update_permissions() {
	...
	echo
	chown -R cyberpot:cyberpot /data
	chmod -R 770 /data
...
if [ "${myOSTYPE}" == "microsoft" ] && [ "${CYBERPOT_OSTYPE}" != "win" ];
  then
    echo "# Docker Desktop for Windows detected, but CYBERPOT_OSTYPE is not set to win."
    echo "# 1. You need to adjust the OSType in the CyberPot .env config."
...
if [ "${myOSTYPE}" == "linuxkit" ] && [ "${CYBERPOT_OSTYPE}" != "mac" ];
  then
    echo "# Docker Desktop for macOS detected, but CYBERPOT_OSTYPE is not set to mac."
    echo "# 1. You need to adjust the OSType in the CyberPot .env config."
    echo "# 2. You need to copy compose/mac_win.yml to ./docker-compose.yml."
    echo
    echo "# Aborting."
...
if [ "${myOSTYPE}" == "" ] && [ "${CYBERPOT_OSTYPE}" != "linux" ];
  then
    echo "# Docker Engine detected, but CYBERPOT_OSTYPE is not set to linux."
    echo "# 1. You need to adjust the OSType in the CyberPot .env config."
...
for var in CYBERPOT_BLACKHOLE CYBERPOT_PERSISTENCE CYBERPOT_ATTACKMAP_TEXT CYBERPOT_ATTACKMAP_TEXT_TIMEZONE CYBERPOT_REPO CYBERPOT_VERSION CYBERPOT_PULL_POLICY CYBERPOT_OSTYPE;
  do
    check_var "$var"
    check_safety "$var"
    validate_format "$var"
...
if [ "${CYBERPOT_TYPE}" == "HIVE" ];
  then
    ...
    if [ "${LS_WEB_USER}" == "" ];
      then
        echo "# Warning: No LS_WEB_USER detected! CyberPots of type SENSOR will not be able to submit logs to this HIVE."
        echo
      else
...
if [ "${CYBERPOT_TYPE}" == "SENSOR" ];
 then
   # No $ for check_var
   check_var "CYBERPOT_HIVE_USER"
   check_var "CYBERPOT_HIVE_IP"
   validate_base64 "$CYBERPOT_HIVE_USER"
   validate_ip_or_domain "$CYBERPOT_HIVE_IP"
   WEB_USER=""
fi
echo
...
if [ -f "/data/uuid" ];
  then
    figlet "Initializing ..."
    figlet "CyberPot: ${CYBERPOT_VERSION}"
    create_web_users
    echo
    echo "# Data folder is present, just cleaning up, please be patient ..."
    echo
    /opt/cyberpot/bin/clean.sh "${CYBERPOT_PERSISTENCE}"
    echo
  else
    figlet "Setting up ..."
    figlet "CyberPot: ${CYBERPOT_VERSION}"
    ...
    echo
    tar xvfz /opt/cyberpot/etc/objects/elkbase.tgz -C /
    uuidgen > /data/uuid
fi

# Check if CYBERPOT_BLACKHOLE is enabled
if [ "${CYBERPOT_OSTYPE}" == "linux" ];
  then
    if [ "${CYBERPOT_BLACKHOLE}" == "ENABLED" ] && [ ! -f "/etc/blackhole/mass_scanner.txt" ];
      then
        ...
        /opt/cyberpot/bin/blackhole.sh add
        echo
    fi
    if [ "${CYBERPOT_BLACKHOLE}" == "DISABLED" ] && [ -f "/etc/blackhole/mass_scanner.txt" ];
      then
        echo
        echo "# Removing Blackhole routes."
        /opt/cyberpot/bin/blackhole.sh del
    ...
    echo
    echo "# CyberPot is configured for macOS / Windows. Blackhole is not supported."
    echo
fi
...
if [ "${CYBERPOT_OSTYPE}" == "linux" ];
  then
    ...
    echo
    /opt/cyberpot/bin/rules.sh ${COMPOSE} set
  else
    echo
    echo "# CyberPot is configured for macOS / Windows. Setting up firewall rules on the host is not supported."
    echo
fi
...
if [ "${CYBERPOT_OSTYPE}" == "linux" ];
  then
    ...
    echo "# Conflicting ports will prevent the startup of CyberPot."
    echo
    netstat -tulpen | grep -Eo ':([0-9]+)' | cut -d ":" -f 2 | uniq
    echo
  else
    echo
    echo "# CyberPot is configured for macOS / Windows. Showing open ports from the host is not supported."
    echo
fi
...
echo
/opt/cyberpot/autoheal.sh autoheal &
PID=$!
wait $PID
echo "# CyberPot Init and Autoheal were stopped. Exiting."
