...
echo
echo "###############################"
echo "# Deploying to CyberPot Hive ... #"
echo "###############################"
echo
sshpass -e ssh -4 -t -T -l "$MY_CYBERPOT_USERNAME" -p 64295 "$MY_HIVE_IP" << EOF
echo "$SSHPASS" | sudo -S bash -c 'useradd -m -s /sbin/nologin -G cyberpotlogs "$MY_HIVE_USERNAME";
...
echo
echo "############################################"
echo "# Checking for CyberPot Hive availability ... #"
echo "############################################"
echo
sshpass -e ssh -4 -t -l "$MY_CYBERPOT_USERNAME" -p 64295 -f -N -L64305:127.0.0.1:64305 "$MY_HIVE_IP" -o "StrictHostKeyChecking=no"
if [ $? -eq 0 ];
  then
    echo
    echo "#########################"
    echo "# CyberPot Hive available! #"
    echo "#########################"
    echo
	...
	echo
        echo "##############################"
        echo "# CyberPot Hive tunnel test OK! #"
        echo "##############################"
        echo
        kill -9 $(pidof ssh)
      else
        echo
	echo "######################################################"
        echo "# CyberPot Hive tunnel test FAILED!                     #"
	echo "# Tunneled port tcp/64305 unreachable on CyberPot Hive. #"
	echo "# Aborting.                                          #"
        echo "######################################################"
        echo
        kill -9 $(pidof ssh)
    ...
    echo
    echo "#################################################################"
    echo "# Something went wrong, most likely CyberPot Hive was unreachable! #"
    echo "# Aborting.                                                     #"
    echo "#################################################################"
    echo
    rm $MY_SENSOR_PUBLICKEYFILE
...
echo
echo "### Please provide data from your CyberPot Hive installation."
echo "### This usually is the one running the 'CyberPot Hive' type."
...
echo "# run /opt/cyberpot/bin/updateip.sh.                          #"
echo "###########################################################"
echo
tee $MY_LS_ENVCONFIGFILE << EOF
MY_CYBERPOT_TYPE=$MY_CYBERPOT_TYPE
...
echo
echo "#################################"
echo "# Ship CyberPot Logs to CyberPot Hive #"
echo "#################################"
echo
echo "If you already have a CyberPot Hive installation running and"
echo "this CyberPot installation is running the type \"Pot\" the"
echo "script will automagically setup this CyberPot to ship and"
echo "prepare the Hive to receive logs from this CyberPot."
echo
echo
echo "###################################"
echo "# Deploy CyberPot Logs to CyberPot Hive #"
echo "###################################"
echo 
