...
read apiToken
printf "[*] If you have multiple CyberPots running, give them each a unique NUMBER, e.g. '2' for your second CyberPot installation. Enter unique number for THIS CyberPot: "
read indexNumber
if ! [[ "$indexNumber" =~ ^[0-9]+$ ]]
    then
...
echo "API URL: " $apiURL
echo "Unique numeric ID for your CyberPot Installation: "  $indexNumber
...
wget -q https://raw.githubusercontent.com/khulnasoft/cyberpot/master/docker/ews/dist/ews.cfg -O ews.cfg.dist 
...
echo "[+] Changing cyberpot.yml to include new ews.cfg."

cp ews.cfg /data/ews/conf/ews.cfg
cp /opt/cyberpot/etc/cyberpot.yml /opt/cyberpot/etc/cyberpot.yml.bak
sed -i '/- \/data\/ews\/conf\/ews.ip:\/opt\/ewsposter\/ews.ip/a\ \ \   - \/data\/ews\/conf\/ews.cfg:\/opt\/ewsposter\/ews.cfg' /opt/cyberpot/etc/cyberpot.yml
