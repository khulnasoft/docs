...
myEXTIP=$(/opt/cyberpot/bin/myip.sh)
if [ "$myEXTIP" = "" ];
  then
    myEXTIP=$myLOCALIP
fi
...
echo "CyberPot 23.12" >> /tmp/etc/issue
echo >> /tmp/etc/issue
...
ip = $myEXTIP
EOF
tee /data/cyberpot/etc/compose/elk_environment << EOF
HONEY_UUID=$myUUID
MY_EXTIP=$myEXTIP
MY_INTIP=$myLOCALIP
...
chown cyberpot:cyberpot /data/ews/conf/ews.ip
