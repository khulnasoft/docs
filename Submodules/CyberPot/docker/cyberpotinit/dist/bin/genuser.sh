#!/usr/bin/env bash
myCYBERPOT_CONF_FILE=/data/.env

# Read WEB_USER from file
WEB_USER=$(grep "^WEB_USER=" "${myCYBERPOT_CONF_FILE}" | sed 's/^WEB_USER=//g' | tr -d "\"'")

myPW=$(cat << "EOF"
__        __   _     _   _  [ CyberPot ]
\ \      / /__| |__ | | | |___  ___ _ __
 \ \ /\ / / _ \ '_ \| | | / __|/ _ \ '__|
  \ V  V /  __/ |_) | |_| \__ \  __/ |
...
# Generate CyberPot WebUser
echo "$myPW"
echo
echo "### This script will ask for and create CyberPot web users."
echo
...
sed -i "s|^WEB_USER=.*|WEB_USER=${WEB_USER}|" ${myCYBERPOT_CONF_FILE}
