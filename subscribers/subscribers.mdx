...
In Teleflow, we call the entities designed to receive notifications as `Subscribers`. Each subscriber is unique and identified by a unique subscriberId.

<Note>
`subscriberId` is a unique identifier used by Teleflow to keep track of a specific subscriber.
...
**Teleflow subscriber object**

```json
{
  "_id": "TELEFLOW_GENERATED_SUBSCRIBER_ID",
  "_organizationId": "TELEFLOW_GENERATED_ORG_ID",
  "_environmentId": "TELEFLOW_GENERATED_ENV_ID",
  "firstName": "John",
  "lastName": "Doe",
  ...
  "channels": [
  {
    ...
    "_integrationId": "TELEFLOW_GENERATED_INTEGRATION_ID",
    "providerId": "fcm"
  },
  {
    "credentials": {
  ...
  "avatar": "AVATAR_URL",
  "id": "TELEFLOW_GENERATED_SUBSCRIBER_ID"
}
```

## Create a subscriber

You can create a subscriber in Teleflow using the `identify` method provided in Teleflow's SDK.
...
// Insert your Teleflow API Key here
const teleflow = new Teleflow("<TELEFLOW_API_KEY>");

// Create a subscriber on Teleflow
await teleflow.subscribers.identify("132", {
...
You should now be able to see your new subscriber on the Teleflow dashboard.

You can also add additional fields such as `phone`, `avatar`, and `data`.

<Note> Remember, all user IDs added to Teleflow need to be unique. </Note>
...
await teleflow.subscribers.identify('111', {
  email: 'john.doe@domain.com',
  firstName: 'John',
  lastName: 'Doe',
  phone: '+13603963366',
...
Here, `mock_data.csv` file is a CSV file having mock data. The below script will read this CSV file and create a new subscriber in Teleflow. Change example variables values like `API_KEY` value with your valid values. Download `mock_data.csv` file from [here](https://drive.google.com/file/d/159Upw30ZcPa_QnzlKF-C6_dHKDJeU-TW/view).
...
API_ENDPOINT="https://api.teleflow.khulnasoft.com/v1/subscribers"
...
fs.createReadStream('mock_data.csv')
  .pipe(csv())
  .on('data', async (data) => {
    let subscriber = await teleflow.subscribers.identify(data.subscriberId, {
      email: data.email,
...
await teleflow.subscribers.delete('111');
```
  </Tab>
  <Tab title="PHP">
```php
...
await teleflow.subscribers.update('111', {
  // new email
...
await teleflow.subscribers.get('111');
```
...
$teleflow->getSubscriber('111')->toArray();
```
