...
Teleflow provides a way to store user preferences in the subscribers data model.
This allows subscribers to specify and manage their preferences, without your intervention. Customizing preferences has become the standard expected behavior for people and Teleflow can take the technical burden of managing preferences off of you.

**Teleflow allows several levels of preferences:**
...
// 111 is subscriberId
$teleflow->getSubscriberPreferences('111')->toArray();
```
...
await teleflow.subscribers.getPreferenceByLevel("subscriberId", PreferenceLevelEnum.GLOBAL)

// Get template level preference
await teleflow.subscribers.getPreferenceByLevel("subscriberId", PreferenceLevelEnum.TEMPLATE);
```
...
// enable in_app channel
$teleflow->updateSubscriberPreference('subscriberId', 'workflowIdentfier', [
    'enabled' => true
    'channel' => [
      'type' => 'in_app',
      'enabled' => true
