---
title: 'VanillaJS'
description: 'Learn how to use Teleflow to quickly send multi-channel (SMS, Email, Chat, Push) notifications with VanillaJS.'
icon: 'js'
---

Learn how to integrate Teleflow into your vanilla JS app on the fly. Send notifications across different channels (SMS, Email, Chat, Push) and enable real-time In-App notifications with the help of a rich and customizable Notification Center.

## Requirements

To follow the steps in this quickstart, you'll need:

- A Teleflow account. [Sign up for free](http://web.teleflow.khulnasoft.com/?utm_campaign=docs-gs-quick-vanilla) if you don’t have one yet.
- A working Vanilla JS development environment.

You can find the code for this project [here](https://github.com/khulnasoft/vanillajs-quickstart)
...
You can find out more about the web component [here](https://docs.teleflow.khulnasoft.com/notification-center/web-component/).
...
<html lang="en">
  <head>
    ...
    <!-- CDN link for Teleflow notification center component -->
    <script src="​https://web-component.teleflow.khulnasoft.com/index.js"></script>
  </head>
  <body>
    <!-- HTML body -->
...
An application identifier is a public key used to identify your application. You can get your own application identifier from the [Teleflow dashboard settings](https://web.teleflow.khulnasoft.com/settings?utm_campaign=docs-quick-vanilla).

And subscribers are users to which notifications will be sent. They are identified by a `subscriberID` which you can also find in the [Teleflow subscribers dashboard](https://web.teleflow.khulnasoft.com/subscribers?utm_campaign=docs-quick-vanilla). Let's learn more about subscribers.

## Subscribers in Teleflow

If you click “Subscriber” on the left sidebar of the [Teleflow dashboard](https://web.teleflow.khulnasoft.com/subscribers?utm_campaign=docs-quick-vanilla), you’ll see the subscriber list. By default, there will only be one subscriber as you’re automatically added as a subscriber when you sign up for Teleflow:
...
Now, let’s create a subscriber on Teleflow. After creating a subscriber, we’ll trigger a notification to this subscriber. Subscribers are identified by a unique `subscriberId`.

With Teleflow, you can create a subscriber using any of its SDKs (Node.js, PHP, .NET, Go, Ruby, Python and Kotlin). The code to create a subscriber in Teleflow is:
...
await teleflow.subscribers.identify('123', {
  firstName: 'Sumit',
  lastName: 'Saurabh',
  returnUser: true,
});
...
You can get your API key from the Teleflow dashboard. Replace `<TELEFLOW_API_KEY>` with it. Now, if you’ll go to the Teleflow dashboard, you shall see the subscriber we created above with `subscriberId` of `123`.
...
await teleflow.subscribers.update('123', {
...
Having created a workflow and having our `subscriberID`, we're ready to send notifications using Teleflow!

## Using Teleflow in a vanilla JS app
...
<html lang="en">
  ...
  <body>
    <div>
      ...
      <notification-center-component
        style="display: inline-flex"
        application-identifier="<YOUR_TELEFLOW_APPLICATION_IDENTIFIER>"
        subscriber-id="123"
      ></notification-center-component>
    </div>
    <div class="">
...
The front-end as well as the back-end code for the sample application can be found [here](https://github.com/khulnasoft/vanillajs-quickstart) for illustration.

## Code to trigger the notification

We can trigger a notification by simply executing the trigger code snippet we get from the Teleflow web dashboard.

To get the snippet:

1. Go to **Workflows** in the left sidebar on Teleflow web dashboard.
