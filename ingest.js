...
// `khulnasoft` and `master`. Setting these are useful in GitHub Actions or
...
async function getRootSha(user = 'khulnasoft', repo = 'khulnasoft', branch = 'master') {
  const { data: { commit: { sha } } } = await ax.get(`${user}/${repo}/branches/${branch}`)
  return sha
}

async function getNodes(rootSha, user = 'khulnasoft', repo = 'khulnasoft') {
...
function normalizeLinks(pages) {
  return pages.map(page => {
    ...
    const body = page.body.replace(/\]\((.*?)\)/gs, (match, url) => {
      ...
      // For links started with `https://docs.khulnasoft.com/`, cut that and
      // leave the rest of the path in place. Return the normalized link.
      if (url.startsWith('https://docs.khulnasoft.com/')) return `](${url.split('https://docs.khulnasoft.com')[1]})`
...
async function ingest() {
  ...
  console.log(`Fetching root SHA for 'khulnasoft' repo...`)
  const rootSha = await getRootSha(user, 'khulnasoft', branch)
  console.log(`Fetching nodes from ${user} repo...`)
  const nodes = await getNodes(rootSha)

  console.log(`Fetching root SHA for '.github' repo...`)
  const ghRootSha = await getRootSha('khulnasoft', '.github', 'main')
  console.log(`Fetching nodes from '.github' repo...`)
  const ghNodes = await getNodes(ghRootSha, 'khulnasoft', '.github')

  console.log(`Fetching root SHA for 'go.d.plugin' repo...`)
  const goRootSha = await getRootSha('khulnasoft', 'go.d.plugin', 'master')
  console.log(`Fetching nodes from 'go.d.plugin' repo...`)
  const goNodes = await getNodes(goRootSha, 'khulnasoft', 'go.d.plugin')

  console.log(`Fetching root SHA for 'agent-service-discovery' repo...`)
  const sdRootSha = await getRootSha('khulnasoft', 'agent-service-discovery', 'master')
  console.log(`Fetching nodes from 'agent-service-discovery' repo...`)
  const sdNodes = await getNodes(sdRootSha, 'khulnasoft', 'agent-service-discovery')
