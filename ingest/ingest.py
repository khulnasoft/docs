"""
This is the script that gathers markdown files from all of Khulnasoft's repos in this repo
...
Stages of this ingest script:
    ...
    Stage_2: We create three buckets:
                ...
                3. toPublish: markdown files that must be included in the docs 
                    (metadata_key_value: "docs_status": "Published") 
...
FAIL_ON_KHULNASOFT_BROKEN_LINKS = False
...
def populate_integrations(markdownFiles):
        ...
        if whole_file not in ignore_dup and "DO NOT EDIT THIS FILE DIRECTLY" in whole_file:
            ...
            if os.path.islink(file):
                ...
                if not file.replace("ingest-temp-folder/", "").split('/', 1)[1] in metadata_dictionary['custom_edit_url']:
                    ...
                    proper_edit_url = "https://github.com/khulnasoft/" + \
            ...
            # here we need a different check, as the path variable gets messed up
                ...
                metadf['docs_rel_path'] = metadf['docs_rel_path'] + "/" + metadf['sidebar_label']
    ...
    map_file = pd.concat([upper, authentication_entries.sort_values(
        by=['docs_rel_path','sidebar_label'], key=lambda col: col.str.lower()), lower], ignore_index=True)
...
def produce_gh_view_link_for_repo(repo, file_path):
    ...
    Limitation it produces only  the master, main links only for the khulnasoft org
    """
    if repo == ".github":
        return f"https://github.com/khulnasoft/{repo}/blob/main/{file_path}"
    else:
        return f"https://github.com/khulnasoft/{repo}/blob/master/{file_path}"
...
def produce_gh_edit_link_for_repo(repo, file_path):
    ...
    """
    if repo == ".github":
        return f"https://github.com/khulnasoft/{repo}/edit/main/{file_path}"
    else:
        return "https://github.com/khulnasoft/{repo}/edit/master/{file_path}"


def safe_cleanup_docs_folders(folder_to_delete):
    """
    Cleanup every file in the specified folderToDelete, that doesn't have the `part_of_docs: True`
    ...
    for md in md_files:
        ...
        try:
            if "part_of_docs" in metadata.keys():
                # Reductant condition to emphasize what we are looking for when we clean up docs files
                if metadata["part_of_docs"] == "True":
                    pass
            else:
...
def create_mdx_path_from_metadata(metadata):
    ...
    if "Collecting Metrics" in metadata['docs_rel_path']\
            and metadata['docs_rel_path'].split("/")[-1] != "Collecting Metrics" and 'External-plugins' not in metadata['docs_rel_path']:
        last_folder = metadata['docs_rel_path'].split("Collecting Metrics", 1)[1]
...
def insert_and_read_hidden_metadata_from_doc(path_to_file, dictionary):
    ...
    if repo == ".github":
        key = "https://github.com/khulnasoft/" + repo + "/edit/main" + "/" + path
    else:
        key = "https://github.com/khulnasoft/" + repo + "/edit/master" + "/" + path
...
def add_new_docs_path_key_to_dict(input_dict, docs_prefix, docs_path_docs, temp_folder):
    """
    This function takes as an argument our dictionary of the Ingest process and creates a new dictionary with key-value
    pairs of type Source file -> Target file (docs_absolute path)
    """
    ...
    for element in input_dict:
        ...
        output_dictionary[source_link] = input_dict[element]["docsPath"]\
            .split(".mdx")[0]\
            .lstrip('"')\
            .rstrip('"')\
            .replace(docs_prefix, docs_path_docs)
        ...
        # If there is a slug present in the file, then that is the new_docs_path, with a "/docs" added in the front.
        try:
            input_dict[element].update(
                {"new_docs_path": "/docs"+input_dict[element]["metadata"]["slug"]})
        except KeyError:
            input_dict[element].update({"new_docs_path": _temp})
                    ...
                    for root, dirs, files in os.walk(directory):
                        for name in files + dirs:
                            if substring in os.path.join(root, name):
                                ...
                                # print(url, path_to_file, path_to_file.split('/')[1], "NOT IN DOCS")

                                replace = f"(https://github.com/khulnasoft/{path_to_file.split('/')[1]}/blob/master{url}"
...
def convert_github_links(path_to_file, input_dict):
    ...
    Expected format of links in files:
        [*](https://github.com/khulnasoft/khulnasoft/blob/master/*)
        or any other Khulnasoft repo
    """
    ...
    if re.search(r"\]\((.*?)\)", body):
        ...
        for url in urls:
            # The URL will get replaced by the value of the replaceString
            try:
                # The keys inside fileDict are like "ingest-temp-folder/khulnasoft/collectors/charts.d.plugin/ap/README.md"
                ...
                dictionary = input_dict[url.replace("https://github.com/khulnasoft", TEMP_FOLDER).replace(
                    "edit/", "blob/", 1).replace("blob/master/", "").replace("blob/main/", "")]

                replace_string = dictionary["new_docs_path"].replace("//", "/")
                ...
                # This is probably a link that can't be translated to a Docs link (e.g. An external file)
                if url.startswith("https://github.com/khulnasoft") and re.search(r"\.md", url):
                    ...
                    if "integrations" in url and ("collector" in url):
                        ...
                        try:
                            ...
                            dictionary = input_dict[try_url.replace("https://github.com/khulnasoft", TEMP_FOLDER).replace(
                                "edit", "blob").replace("blob/master/", "").replace("blob/main/", "")]
                            replace_string = dictionary["new_docs_path"]
...
def automate_sidebar_position(dictionary):
    ...
    for path in dictionary['docs_rel_path']:
        if str(path) != "nan":
...
if __name__ == '__main__':
    ...
    # khulnasoft/khulnasoft:branch tkatsoulas/go.d.plugin:mybranch
    ...
    for arg in kArgs:
        ...
        if arg[0] == "fail_on_internal_broken_links":
            FAIL_ON_KHULNASOFT_BROKEN_LINKS = arg[1]
    ...
    safe_cleanup_docs_folders(DOCS_PREFIX)
    ...
    for markdown in all_markdown_files:
        ...
        if len(md_metadata) > 0:
            ...
            if "docs_status" in md_metadata.keys() and md_metadata["docs_status"] == "Published":
                try:
                    ...
                    if type(response) != str:
                        ...
                        to_publish[markdown] = {
                            ...
                            "docsPath": str(response[0]),
                            "ingestedRepo": str(markdown.split("/", 2)[1])
                        }

                        md_metadata.update(
                            {"docs_link": "https://docs.khulnasoft.com/docs" + md_metadata['slug']})

                    else:
                        to_publish[markdown] = {
                            "metadata": md_metadata,
                            "docsPath": str(response),
                            "ingestedRepo": str(markdown.split("/", 2)[1])
                        }
                        # replace first ", " and then " ", this needs to be handled in a prettier way, but other updates in this file are on the way.
                        if md_metadata['docs_rel_path'] != md_metadata['sidebar_label']:
                            md_metadata.update({"docs_link": "https://docs.khulnasoft.com/docs/" + clean_and_lower_string(
                                md_metadata['docs_rel_path']) + "/" + clean_and_lower_string(md_metadata['sidebar_label'])})
                        else:
                            md_metadata.update(
                                {"docs_link": "https://docs.khulnasoft.com/docs/" + clean_and_lower_string(md_metadata['docs_rel_path'])})
                ...
                rest_files_with_metadata_dictionary[markdown] = {
                    "metadata": md_metadata,
                    "docsPath": str(f"docs/_archive/_{markdown}"),
                    "ingestedRepo": str(markdown.split("/", 2)[1])
                }
    ...
    print(f"### Found Docs files: {len(to_publish)}###\n")

    for md_file in to_publish:
        local_to_absolute_links(md_file, to_publish)
        copy_doc(md_file, to_publish[md_file]["docsPath"])
        sanitize_page(to_publish[md_file]["docsPath"])

    print("### Fixing github links ###")

    # After the moving, we have a new metadata, called new_docs_path, and we utilize that to fix links that were
    # pointing to GitHub relative paths
    file_dict = add_new_docs_path_key_to_dict(
        to_publish, DOCS_PREFIX, "/docs", TEMP_FOLDER)

    for md_file in to_publish:
        convert_github_links(file_dict[md_file]["docsPath"], file_dict)

    genRedirects.main(file_dict)
    print("Done.", "Uncorrelated links (links from our github repos that the files are not in Docs):",
          UNCORRELATED_LINK_COUNTER)

    if DEBUG:
        # Print the list of markdown not in Docs, for debugging purposes
