...
docs_topic_type: "Getting started"
docs_rel_path: "Getting started"
docs_docs_purpose: "Present all the KhulnaSoft integrations"
docs_doc_type: "AUTOGENERATED"
---

KhulnaSoft Solution support many integrations, this document will guide you through all the integrations. We categorize
...
'''
	...
	for key in index.keys():
		refPrefix = ''
		if len(index[key]["metadata"]["docs_rel_path"].split("References/"))>1:
			refPrefix = index[key]["metadata"]["docs_rel_path"].lower().split("references/")[1]
			if refPrefix.__contains__("collectors"):
				collectorCategory = index[key]["metadata"]["docs_rel_path"].lower().split("references/collectors references/")[1]
	...
	for key in orderedNotifactionsBucket:
		partOfCloud = unpackDictionaryStringToDictionary(orderedNotifactionsBucket[key]["metadata"]["docs_autogeneration_metadata"])["part_of_cloud"]
...
"""
	...
	for key in orderedeExportersBucket:
		partOfCloud = unpackDictionaryStringToDictionary(orderedeExportersBucket[key]["metadata"]["docs_autogeneration_metadata"])["part_of_cloud"]
		if partOfCloud == False:
