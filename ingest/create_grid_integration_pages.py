...
def sort_files(file_array):
    ...
    for file in file_array:
        if Path(file).is_file():
            ...
            if "most_popular: \"True\"" in content:
                most_popular.append(
                    [str(file).lower().rsplit("/", 1)[1], file, "by Khulnasoft",  "#00ab44"])
            elif "maintained%20by-Khulnasoft-" in content:
                rest_khulnasoft_integrations.append(
                    [str(file).lower().rsplit("/", 1)[1], file, "by Khulnasoft", "#00ab44"])
            else:
    ...
    sorted_array = sorted(
        most_popular) + sorted(rest_khulnasoft_integrations) + sorted(community_integrations)
...
def get_dir_make_file_and_recurse(directory):
    ...
    if len(sorted(Path(directory).glob("**/**/*"))) > 1 or directory == "docs/khulnasoft-cloud/authentication-&-authorization/cloud-authentication-&-authorization-integrations":
        # print(directory)
...
docs_link: "https://docs.khulnasoft.com/{ingest_func.clean_and_lower_string(str(directory))}"
---
        ...
        for file_array_entry in sorted_list:
            ...
            if Path(file).is_file():
                ...
                if "DO NOT EDIT THIS FILE DIRECTLY" in whole_file:
                    ...
                    if direct_child and is_only_integration and meta_dict["docs_link"].split("/")[-1] == meta_dict["docs_link"].split("/")[-2]:
                        break
                    ...
                    if meta_dict["docs_link"].split("/")[-1] == meta_dict["docs_link"].split("/")[-2] and not is_only_file:
                        meta_dict["docs_link"] = meta_dict["docs_link"].rsplit(
                            "/", 1)[0]
                        print("IN", type(meta_dict["docs_link"]), file_array_entry, directory, direct_child
                              )

                    # print(file)

                    try:
                        img = re.search(r'<img src="https:\/\/khulnasoft.com\/img.*', whole_file)[0].replace(
                    ...
                    md += \
                        f"""
<Box banner="{message}" banner_color="{color}" to="{meta_dict["docs_link"].replace("https://docs.khulnasoft.com", "")}"  title="{meta_dict["sidebar_label"]}">
    {img}
</Box>
"""
                    integrations += 1
...
get_dir_make_file_and_recurse(
    'docs/khulnasoft-cloud/authentication-&-authorization/cloud-authentication-&-authorization-integrations')
get_dir_make_file_and_recurse(
    'docs/logs')
