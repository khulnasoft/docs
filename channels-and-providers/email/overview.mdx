...
description: 'Learn the process of configuring email providers with Teleflow'
---

Teleflow can be used to deliver email messages to your customers using a unified delivery API. You can easily integrate your favorite email provider using the built-in integration store.
...
- **From email address** - Emails sent using Teleflow will be sent using this address
...
teleflow.trigger('<WORKFLOW_TRIGGER_IDENTIFIER>', {
  ...
  overrides: {
    email: {
      to: ['to@teleflow.khulnasoft.com'],
      from: 'from@teleflow.khulnasoft.com',
      senderName: 'Teleflow Team',
      text: 'text version of email using overrides',
      replyTo: 'no-reply@teleflow.khulnasoft.com',
      cc: ['1@teleflow.khulnasoft.com'],
      bcc: ['2@teleflow.khulnasoft.com'],
    },
  },
});
```
...
<Note>It's very important to know that Teleflow merges the `to` field in the email overrides with the subscriber email. It DOES NOT REPLACE IT.</Note>

## Sending Email attachments

You can easily send attachments with the Teleflow API by passing the attachments array when triggering an Email based workflow. Attachment file can either be in the `buffer` or `base64` format.
...
teleflow.trigger('<WORKFLOW_TRIGGER_IDENTIFIER>', {
  ...
  payload: {
    attachments: [
      {
        file: fs.readFileSync(__dirname + '/data/teleflow.jpeg'),
        name: 'teleflow.jpeg',
        mime: 'image/jpeg',
      },
    ],
...
In Teleflow integration store, multiple email channel type provider integrations can be active at the same time. But only one  provider integration can be primary at a time. This primary integration will be used as a provider to deliver the email by default. If you want to use a different active provider integration then you can use the `integrationIdentifier` email overrides field.
