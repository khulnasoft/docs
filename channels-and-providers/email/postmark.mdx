---
title: 'Postmark'
description: 'Learn how to use the Postmark provider to send email notifications using Teleflow'
---

It is possible to use [Postmark](https://postmarkapp.com/) as a provider to send transactional emails to your customers using the Teleflow Platform with a single API.

# Getting Started

The first step to use the Postmark provider in the email channel is to create a Postmark account and add the personal API key to the Postmark integration on the Teleflow platform.
...
# Create a Postmark integration with Teleflow

- Visit the [Integrations](https://web.teleflow.khulnasoft.com/integrations?utm_campaign=docs-postmark) page on Teleflow.
...
- Now is possible to send notifications using Postmark in Teleflow.