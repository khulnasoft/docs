...
2. Update the subscriber credentials with this URL using the MS Teams provider id. You can do this step by using the `@teleflow/node` library, as shown below:
...
await teleflow.subscribers.setCredentials('subscriberId', ChatProviderIdEnum.MsTeams, {
...
1. You are all set up and ready to send your first chat message via our `@teleflow/node` package or directly using the REST API.