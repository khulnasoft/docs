---
title: "Push Webhook"
description: "Learn how to use the Push Webhook provider to send  notifications using Teleflow"
---
...
Users can use their own api url as webhook url and teleflow will make a post request on that webhook url.

## Steps To Configure

1. Go to [integration store](https://web.teleflow.khulnasoft.com/integrations?utm_campaign=docs-push-webhook) and click on `Add a provider` button. Choose `Push` channel and then `Push Webhook` provider.
2. Enter your Webhook URL. For quick testing use [this](https://webhook.site/) website.
3. Enter Secret Hmac Key. Teleflow will use this secret hmac key to encrypt the data using `HMAC SHA256` algorithm and send the hash as value of `x-teleflow-signature` header. User can use `x-teleflow-signature` header to test authenticity of the request. Read more [here](#checking-authenticity)
...
await teleflow.subscribers.setCredentials('subscriberId', PushProviderIdEnum.PushWebhook, {
...
## Example paylod sent by teleflow to webhook url 
...
async acceptTeleflowPushWebHookRequest(request, response){

  const payloadSentByTeleflow = request.body
  const hmacHashSentByTeleflow = request.headers['x-teleflow-signature']

  const actualHashValue = crypto
    .createHmac('sha256', secretKey)
    .update(payloadSentByTeleflow, 'utf-8')
    .digest('hex');

  if(hmacHashSentByTeleflow === actualHashValue){
    // handle the notification
    console.log("Request sent by Teleflow")
  } else {
