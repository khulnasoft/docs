...
sidebarTitle: "FCM"
description: "Learn how to use the Firebase Cloud Messaging (FCM) provider to send push notifications using Teleflow"
---
...
After that, paste the entire JSON file content in the Service Account field of the FCM provider in the integration store on Teleflow’s web dashboard.
...
<Info> Teleflow uses FCM version V1</Info>

## Relative Link in Webpush

Suppose you’re using the Firebase (FCM) provider to send push notifications to web browsers via Teleflow and want users to be returned to the website after clicking the notification.
...
```
  ...
  <Tab title="Curl">
    ```bash
    curl --location --request POST 'https://url.to.our.selfhosted.teleflow' \
