---
title: 'Pusher Beams'
description: 'Learn how to use the Pusher Beams provider to send push notifications using Teleflow'
---
...
In order to target the Pusher Beams user from Teleflow, you must register the Pusher Beams Authenticated Users with their `userId` as the `deviceToken` for your Teleflow subscriber. This value can be retrieved via [getUserId()](https://pusher.com/docs/beams/reference/web/#getuserid) method from the SDK for your platform.

Once you have the user's `userId` value, the `deviceToken` for your Teleflow subscriber can be set via:
...
await teleflow.subscribers.setCredentials('subscriberId', PushProviderIdEnum.PusherBeams, {
