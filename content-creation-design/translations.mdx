...
<Info>The feature is only available for **Teleflow Cloud** users with **Business** or **Enterprise** plans.</Info>
...
To understand how Translation Management works in Teleflow we need to review its main pillars:
...
To localize your message content, you need to use the translation variables in the editor. The variables allow you to reference the values from the translation files. It consists of the `i18n` [handlebar](https://docs.teleflow.khulnasoft.com/content-creation-design/handlebars-helpers) helper and path to the translated value.

Let’s take a look at the example: `{{i18n "marketing.welcome_message"}}`.

- `i18n` is the handlebar helper that tells Teleflow that you would like to use the Translations
...
To update the locale you can use Teleflow API or any of our SDKs, more information can be found [here](https://docs.teleflow.khulnasoft.com/subscribers/subscribers).
...
```handlebars
{{i18n "welcome.withVariables" name=subscriber.firstName organization="Teleflow"}}
```
