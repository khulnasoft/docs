---
title: "Layouts"
description: "Explore how to create and manage layouts in Teleflow"
icon: "table-layout"
---
...
By default, Teleflow will create a default layout and assign it as the organization's default layout. At any time, you can choose any of your layouts as the default for your organization. All new email templates will be assigned the default layout unless assigned a different one through the email editor.
...
teleflow.trigger('workflow-identifier', {
  to: {
...
Teleflow SDK supports all layout functionalities:
...
const { _id: layoutId } = await teleflow.layouts.create({
  name,
  description,
  content,
  variables,
...
const updatedLayout = await teleflow.layouts.update(layoutId, {
  name,
  description,
  content,
  variables,
...
const layout = await teleflow.layouts.get(layoutId);
```
...
const layouts = await teleflow.layouts.list({
  page,
  pageSize,
  sortBy,
  orderBy,
