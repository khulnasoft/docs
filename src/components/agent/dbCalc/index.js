    ...
    <div className={clsx('row', styles.dbCalc)}>
      <div className={'col col--12'}>
        ...
        <div className={clsx('row', styles.calcRow)}>
          ...
          <div className={clsx('col col--10', styles.calcInstruction)}>
            ...
            <p>By default, the Agent collects metrics once per second (<code>1</code>). See the <code>update every</code> setting in your <code>khulnasoft.conf</code> file if yours may be different. If you have streaming nodes, use the average of their settings.</p>
          </div>
        </div>
        ...
        <div className={clsx('row', styles.calcRow)}>
          ...
          <div className={clsx('col col--10', styles.calcInstruction)}>
            ...
            <p>Open your <code>khulnasoft.conf</code> file and find the <code>page cache size</code> setting in the <code>[global]</code> section.</p>
          </div>
        </div>
      </div>
      ...
      <div className={clsx("col col--12", styles.calcResults)}>
        ...
        <div className={styles.calcFinal}>
          <p>With the above configuration, Khulnasoft will use the following resources{state.child > 0 && <em>&nbsp;on your parent node</em>} to store metrics{state.child > 0 && <span>&nbsp;for both parent and child nodes</span>}: </p>
          <span>
        ...
        <div className={styles.calcConfig}>
          <p>To enable this setup, <Link to="/docs/configure/nodes#use-edit-config-to-edit-khulnasoftconf">edit your <code>khulnasoft.conf</code></Link> file&nbsp;
            {state.child > 0 &&
          ...
          <p>Restart your Agent with <code>service khulnasoft restart</code> for the setting to take effect.</p>
          {state.child > 0 ? (
