...
export function OneLineInstallWget() {
  ...
  let currentCommand = `wget -O /tmp/khulnasoft-kickstart.sh https://get.khulnasoft.com/kickstart.sh && sh /tmp/khulnasoft-kickstart.sh${currentCommandUpdates}${currentCommandRelease}${currentCommandStatistics}${currentCloudOption}`;
  const lang = `bash`
      ...
      <div className="z-10 relative -t-2 p-6 border-l border-b border-r border-gray-200 rounded-br rounded-bl dark:bg-gray-darkbg dark:border-gray-500">
        ...
        <div className="py-1 flex items-center">
          <input
            ...
            type="checkbox"
            id="toggle__cloud" />
          <label htmlFor="toggle__cloud" className="relative text-sm pl-2">Do you want to <Link to="/docs/getting-started/monitor-your-infrastructure/connect-agent-to-cloud" className="hover:text-blue">connect</Link> the node to Khulnasoft Cloud?<code>default: disabled</code></label>
        </div>
      </div>
    </div>
  )
...
export function OneLineInstallCurl() {
  ...
  let currentCommand = `curl https://get.khulnasoft.com/kickstart.sh > /tmp/khulnasoft-kickstart.sh && sh /tmp/khulnasoft-kickstart.sh${currentCommandUpdates}${currentCommandRelease}${currentCommandStatistics}${currentCloudOption}`;
  const lang = `bash`
    ...
    <div className={clsx('relative overflow-hidden mt-8 mb-8 rounded-tr rounded-tl', styles.Container)}>
      ...
      <div className="z-10 relative -t-2 p-6 border-l border-b border-r border-gray-200 rounded-br rounded-bl dark:bg-gray-darkbg dark:border-gray-500">
        ...
        <div className="py-1 flex items-center">
          ...
          <label htmlFor="toggle__cloud_curl" className="relative text-sm pl-2">Do you want to <Link to="/docs/getting-started/monitor-your-infrastructure/connect-agent-to-cloud" className="hover:text-blue">connect</Link> the node to Khulnasoft Cloud?<code>default: disabled</code></label>
        </div>
      </div>
    </div>
  )
